...
# ============================================================================
# Download/resolve an ASR checkpoint from ORD, then run ORACLE time-warp
# ("speed cheat") eval over the Open-ASR-Leaderboard datasets.
#
# This is the oracle-selection sibling of eval_likelihood_timewarp_ord.sh /
# eval_asr_ord.sh. It reuses the exact same local checkpoint cache, ORD layout,
# optional checkpoint averaging, and tokenizer download logic, then delegates the
# leaderboard loop to:
#   ./eval_oracle_timewarp.sh
#
# Per dataset it reports baseline (x1.0) WER, best single fixed warp, the
# LM-selected WER (non-cheating: pick the warp whose transcript scores highest
# under a lightweight LM, distilgpt2 by default), and the ORACLE best-of-N WER
# (per-utterance pick the lowest-WER warp -- cheating upper bound).
#
# Usage:
#   ./eval_oracle_timewarp_ord.sh <EXP_NAME> [STEP] [DEVICE_ID]
#     STEP=last (or USE_LAST=1) -> use the rolling -last.ckpt
#     no STEP                  -> prefer a .nemo, else best non-last .ckpt
#   RUN_AVERAGING=1 ./eval_oracle_timewarp_ord.sh <EXP_NAME>   # average non-last ckpts
#
#   MODEL=/abs/path/model.nemo ./eval_oracle_timewarp_ord.sh
#
# For the spdavg2dev0p3 RNN-T(TDT) model, the typical invocation is:
#   RUN_AVERAGING=1 ./eval_oracle_timewarp_ord.sh \
#       ord_rnnt_parakeetTdt0p6b_granary1p1_ft_lr1e-4_warmup5000_spdavg2dev0p3_n2
#
# Common env overrides (forwarded to eval_oracle_timewarp.sh):
#   FACTORS=0.9,1.0,1.1
...
#   OUT_DIR=oracle_results
#   LM_MODEL=distilgpt2     # LM for non-cheating selection; set LM_MODEL="" to disable
#   LM_EPSILON=0.0          # stay on x1.0 unless another warp's LM score beats it by >= this
#   LM_BATCH_SIZE=16
# ============================================================================
...
LOCAL_DRIVER="${NEMO_ROOT}/eval_oracle_timewarp.sh"
...
if [ ! -x "$LOCAL_DRIVER" ]; then
    echo "ERROR: cannot execute oracle driver at ${LOCAL_DRIVER}" >&2
    exit 1
fi
...
echo "==> Starting ORACLE time-warp leaderboard eval..."
