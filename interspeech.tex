...
Prior work has also studied convolution designs tailored to chunk-wise
Conformers, including dynamic chunk convolution (DCConv)~\cite{li2023dynamic}
and chunked causal convolution (C2Conv) in SSCFormer~\cite{chunkssscformer}.
...
processing time.
DCConv~\cite{li2023dynamic} exploits this observation by adapting convolutional
context to the active chunk, while SSCFormer~\cite{chunkssscformer} combines
sequentially sampled training chunks with C2Conv to further improve chunk-wise
streaming ASR. In contrast, BACON focuses on a parameter-neutral split-channel
replacement for causal convolution that preserves full causal history in part
of the channels while adding chunk-confined right context in the remainder.
...
confined within each chunk (applying this to \emph{all} channels yields the \textbf{all-bidir} baseline we evaluate in Section~\ref{subsec:analysis-zero-vs-split}); and panel~(d) shows BACON's combined dependency. Blue dashed arrows in~(d) highlight the additional
...
chunk size~4) for 12 frames across three chunks.
...
In all result tables, $p$ denotes the two-sided significance of comparison, computed with a paired bootstrap test over the per-utterance outputs; we treat $p<0.05$ as significant.
...
\begin{table}[h]
\centering
\caption{WER~(\%) on LibriSpeech ASR.}
\label{tab:libri_wer}
\begin{tabular}{llccc}
\toprule
\textbf{Family} & \textbf{Test set} & \textbf{Causal}$\downarrow$ & \textbf{BACON}$\downarrow$ & $p$ \\
\midrule
\textsc{rnn-t} & test-clean & 3.36 & \textbf{3.30} & 0.42 \\
\textsc{rnn-t} & test-other & 8.73 & \textbf{8.41} & 0.011 \\
\midrule
\textsc{chat} & test-clean & 3.31 & \textbf{3.09} & 0.005 \\
\textsc{chat} & test-other & 8.47 & \textbf{7.68} & $<$0.001 \\
\bottomrule
\end{tabular}
\end{table}


As shown in Table~\ref{tab:libri_wer}, on LibriSpeech \texttt{test-clean} BACON and Causal achieve similar accuracy---the small gains are not statistically significant for \textsc{rnn-t}---which is expected given the relatively clean conditions. 
On the more challenging \texttt{test-other} set, BACON yields larger and statistically significant gains in both families, especially for \textsc{chat}, where it improves WER by 0.79 absolute over the causal baseline ($8.47\,{\rightarrow}\,7.68$), a 9.3\% relative WER reduction ($p<0.001$).
...
\begin{table}[h]
\centering
\caption{BLEU on English-to-German AST. }
\label{tab:chunk_conv_bleu}
\begin{tabular}{llccc}
\toprule
\textbf{Family} & \textbf{Test set} & \textbf{Causal}$\uparrow$ & \textbf{BACON}$\uparrow$ & $p$ \\
\midrule
\textsc{rnn-t} & \textsc{must-c} & 24.60 & \textbf{25.61} & 0.006 \\
\textsc{rnn-t} & \textsc{covost} & 33.64 & \textbf{34.98} & $<$0.001 \\
\midrule
\textsc{chat} & \textsc{must-c} & 28.04 & \textbf{28.75} & 0.008 \\
\textsc{chat} & \textsc{covost} & 36.40 & \textbf{37.72} & $<$0.001 \\
\bottomrule
\end{tabular}
\end{table}

As detailed in Table~\ref{tab:chunk_conv_bleu}, BACON improves over the causal baseline on both evaluation sets in both families:
+1.32 BLEU on \textsc{covost} for \textsc{chat} and +1.34 for \textsc{rnn-t},
with \textsc{must-c} gains of +0.71 and +1.01, respectively. All four improvements are statistically significant ($p<0.01$).
...
We further assess BACON on a two-speaker ASR task.
...
\begin{table}[h]
\centering
\caption{cpWER~(\%) on 2-speaker ASR.}
\label{tab:multispk}
\begin{tabular}{lcc}
\toprule
...
Table~\ref{tab:multispk} shows BACON reduces cpWER from 27.41\% to 27.14\%. This gain is in the same direction as other tasks, though less significant ($p=0.19$). We hypothesize that cpWER on this task is dominated by challenging conversational phenomena such as overlapping speech and speaker-attribution errors, and per-utterance variability is high, which makes a small acoustic-level improvement harder to establish as significant. Importantly, BACON does not hurt performance and trends slightly better, confirming that the method generalizes to the multi-speaker setting without any regression.
...
A natural alternative to BACON's split design is to apply the boundary-aware
bidirectional convolution of Figure~\ref{fig:chunk_conv_deps}(c) to \emph{all}
depthwise channels rather than to only half of them. We refer to this variant as
\textbf{all-bidir}. It corresponds exactly to panel~(c)---left context crosses
chunk boundaries freely while right context is confined within each chunk---and
is therefore the boundary-aware counterpart to BACON (panel~(d)), which keeps
half of the channels causal. It also represents the closest DCConv-style
comparison in our setting: every depthwise channel uses chunk-confined
bidirectional context, without BACON's causal channel group. This comparison
directly isolates the contribution of BACON's dual-mode (split) design against
the fully boundary-aware variant.
Tables~\ref{tab:analysis-zero-vs-split-libri}
...
\begin{tabular}{llccc}
\toprule
\textbf{Family} & \textbf{Test set} & \textbf{all-bidir}$\downarrow$ & \textbf{BACON}$\downarrow$ & $p$ \\
\midrule
\textsc{chat} & test-clean & 3.25 & \textbf{3.09} & 0.018 \\
\textsc{chat} & test-other & 8.21 & \textbf{7.68} & $<$0.001 \\
\midrule
\textsc{rnn-t} & test-clean & 3.47 & \textbf{3.30} & 0.024 \\
\textsc{rnn-t} & test-other & 8.78 & \textbf{8.41} & 0.003 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{llccc}
\toprule
\textbf{Family} & \textbf{Test set} & \textbf{all-bidir}$\uparrow$ & \textbf{BACON}$\uparrow$ & $p$ \\
\midrule
\textsc{chat} & \textsc{must-c} & 28.41 & \textbf{28.75} & 0.26 \\
\textsc{chat} & \textsc{covost} & 36.32 & \textbf{37.72} & $<$0.001 \\
\midrule
\textsc{rnn-t} & \textsc{must-c} & 24.89 & \textbf{25.61} & 0.018 \\
\textsc{rnn-t} & \textsc{covost} & 33.38 & \textbf{34.98} & $<$0.001 \\
\bottomrule
\end{tabular}
\end{table}

BACON consistently outperforms all-bidir, with significant gains on the more
challenging conditions. On LibriSpeech
\texttt{test-other}, BACON beats all-bidir by $-$0.53 WER for \textsc{chat} and
$-$0.37 for \textsc{rnn-t}. On \textsc{covost}, BACON leads by +1.40 BLEU
(\textsc{chat}) and +1.60 (\textsc{rnn-t}).
Critically, for \textsc{rnn-t} on LibriSpeech, all-bidir \emph{degrades} relative
to the causal baseline (8.78\% vs.\ 8.73\%), whereas BACON improves it
(8.41\%). 
...
\begin{table}[h]
\centering
\caption{LibriSpeech WER~(\%) achieved by
Conformers with unrestricted attention and different types of convolutions.}
...
As shown in Table~\ref{tab:analysis-fullctx}, even with full-utterance attention, non-causal convolution reduces WER by 0.93
...
where $i_j$ is the chunk the token is emitted in. Averaging over all
$N$ emitted tokens:
...
emissions and $\bar{\tau} = \frac{1}{N}\sum_j \tau_j$ the average
ground-truth timestamp. When comparing two models~A and~B evaluated on the
same utterances, the term $\bar{\tau}$ and the constant
...
ideas, methodology, experiments, analysis, or results. All AI-assisted text was
manually reviewed and verified by the authors to ensure its correctness and that
it faithfully reflects the underlying work.
