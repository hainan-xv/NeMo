...
from nemo.collections.asr.metrics.lm_rnnt_wer_bpe import LMRNNTBPEWER, LMRNNTBPEDecoding, LMRNNTBPEDecodingConfig
from nemo.collections.asr.models.lm_rnnt_models import EncDecLMRNNTModel
from nemo.collections.asr.parts.mixins import ASRBPEMixin
from nemo.core.classes.common import PretrainedModelInfo
from nemo.utils import logging, model_utils


class EncDecLMRNNTBPEModel(EncDecLMRNNTModel, ASRBPEMixin):
    """Base class for encoder decoder LM RNNT-based models with subword tokenization."""
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        with open_dict(cfg.lm):
            cfg.lm.vocab_size = len(vocabulary)
        ...
        self.decoding = LMRNNTBPEDecoding(
            decoding_cfg=self.cfg.decoding, decoder=self.decoder, lm=self.lm, joint=self.joint, tokenizer=self.tokenizer,
        )
    ...
    def change_vocabulary(
        ...
        new_lm_config.vocab_size = len(vocabulary)
        del self.lm
        self.lm = EncDecRNNTBPEModel.from_config_dict(new_lm_config)
    ...
    def change_decoding_strategy(self, decoding_cfg: DictConfig):
        ...
        self.decoding = RNNTBPEDecoding(
            decoding_cfg=decoding_cfg, decoder=self.decoder, lm=self.lm, joint=self.joint, tokenizer=self.tokenizer,
        )
