...
from nemo.collections.asr.metrics.lm_rnnt_wer import LMRNNTWER, LMRNNTDecoding, LMRNNTDecodingConfig
...
class EncDecLMRNNTModel(ASRModel, ASRModuleMixin, ExportableEncDecModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.preprocessor = EncDecLMRNNTModel.from_config_dict(self.cfg.preprocessor)
        self.encoder = EncDecLMRNNTModel.from_config_dict(self.cfg.encoder)

        # Update config values required by components dynamically
        with open_dict(self.cfg.decoder):
            self.cfg.decoder.vocab_size = len(self.cfg.labels)

        with open_dict(self.cfg.lm):
            self.cfg.lm.vocab_size = len(self.cfg.labels)
        ...
        self.decoder = EncDecLMRNNTModel.from_config_dict(self.cfg.decoder)
        self.lm = EncDecLMRNNTModel.from_config_dict(self.cfg.lm)
        self.joint = EncDecLMRNNTModel.from_config_dict(self.cfg.joint)
        ...
        if hasattr(self.cfg, 'spec_augment') and self._cfg.spec_augment is not None:
            self.spec_augmentation = EncDecLMRNNTModel.from_config_dict(self.cfg.spec_augment)
        else:
            self.spec_augmentation = None

        # Setup decoding objects
        self.decoding = LMRNNTDecoding(
            decoding_cfg=self.cfg.decoding, decoder=self.decoder, joint=self.joint, lm=self.lm, vocabulary=self.joint.vocabulary,
        )
    ...
    def change_vocabulary(self, new_vocabulary: List[str], decoding_cfg: Optional[DictConfig] = None):
        ...
        if self.joint.vocabulary == new_vocabulary:
            ...
            del self.joint
            self.joint = EncDecLMRNNTModel.from_config_dict(new_joint_config)

            decoder_config = self.decoder.to_config_dict()
            new_decoder_config = copy.deepcopy(decoder_config)
            new_decoder_config.vocab_size = len(new_vocabulary)
            del self.decoder
            self.decoder = EncDecLMRNNTModel.from_config_dict(new_decoder_config)

            lm_config = self.lm.to_config_dict()
            new_lm_config = copy.deepcopy(lm_config)
            new_lm_config.vocab_size = len(new_vocabulary)
            del self.lm
            self.lm = EncDecLMRNNTModel.from_config_dict(new_lm_config)
            ...
            decoding_cls = OmegaConf.structured(LMRNNTDecodingConfig)
            decoding_cls = OmegaConf.create(OmegaConf.to_container(decoding_cls))
            decoding_cfg = OmegaConf.merge(decoding_cls, decoding_cfg)

            self.decoding = LMRNNTDecoding(
                decoding_cfg=decoding_cfg, decoder=self.decoder, lm=self.lm, joint=self.joint, vocabulary=self.joint.vocabulary,
            )
            ...
            logging.info(f"Changed decoder and lm to output to {self.joint.vocabulary} vocabulary.")
    ...
    def training_step(self, batch, batch_nb):
        ...
        lm, lm_target_length, lm_states = self.lm(targets=transcript, target_length=transcript_len)
        ...
        if not self.joint.fuse_loss_wer:
            ...
            joint = self.joint(encoder_outputs=encoded, decoder_outputs=decoder, lm_output=lm)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if not self.joint.fuse_loss_wer:
            if self.compute_eval_loss:
                decoder, target_length, states = self.decoder(targets=transcript, target_length=transcript_len)
                lm, lm_target_length, lm_states = self.lm(targets=transcript, target_length=transcript_len)
                joint = self.joint(encoder_outputs=encoded, decoder_outputs=decoder, lm_outputs=lm)
            ...
            if self.compute_eval_loss:
                ...
                lm, lm_target_len, lm_states = self.lm(targets=transcript, target_length=transcript_len)
            else:
                decoded = None
                lm = None
    ...
    def list_export_subnets(self):
        return ['encoder', 'decoder_joint', 'lm']
