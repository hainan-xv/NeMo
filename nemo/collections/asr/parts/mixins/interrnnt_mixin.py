...
class InterRNNTMixin:

    def _process_rnnt_config_values(self, loss_names: List[str], loss_weights: List[float], apply_at_layers: List[int]):
        self.set_interrnnt_param('loss_names', loss_names)
        self.set_interrnnt_param('intermediate_loss_weights', loss_weights)
        self.set_interrnnt_param('apply_at_layers', apply_at_layers)
        self.set_interrnnt_param('main_loss_weight', 1.0 - sum(loss_weights))
        if self.get_interrnnt_param('main_loss_weight') <= 0.0:
            raise ValueError(
                ...
                "E.g., if interrnnt.loss_weights = [0.1, 0.3], regular "
                "loss will have weight of 0.6"
            )
#        self.set_interrnnt_param('enabled', len(loss_weights) > 0)
        self.set_interrnnt_param('enabled', True)

        if len(apply_at_layers) != len(loss_weights):
            raise ValueError('Length of interrnnt.apply_at_layers has to match interrnnt.loss_weights')

        # setting up config for AccessMixin that will be checked in encoders to
        # log the layers we need
        AccessMixin.update_access_cfg({'interrnnt': {'capture_layers': apply_at_layers}})


    def setup_interrnnt(self, decoder_name, loss_name, wer_name):
        """Sets up all interrnnt-specific parameters and checks config consistency.

        Caller has to specify names of attributes to perform RNNT-specific WER,
        ...
        # main class's names
        self._interrnnt_params = {}
        interrnnt_config = self.cfg.get("interrnnt")
        if interrnnt_config is not None:
            # if interrnnt is in the config, we want to check that it indeed defines
            ...
            self._process_rnnt_config_values(**interrnnt_config)
            self._interrnnt_params['decoder_name'] = decoder_name
            self._interrnnt_params['loss_name'] = loss_name
            self._interrnnt_params['wer_name'] = wer_name
        else:
            self.set_interrnnt_param('enabled', False)
            assert(0)
        ...
        self.inter_rnnt_decoding = RNNTBPEDecoding(   # can't have it here since it needs joint
            decoding_cfg=self.cfg.decoding, decoder=self.inter_rnnt_decoder, joint=self.inter_joint, tokenizer=self.inter_tokenizer,
            )

    def get_interrnnt_param(self, param_name):
        """Either directly get parameter from ``self._interrnnt_params`` or
        call getattr with the corresponding name.
        """
        if param_name in ['decoder', 'loss', 'wer']:
            return getattr(self, self._interrnnt_params[param_name + "_name"])
        return self._interrnnt_params[param_name]

    def set_interrnnt_param(self, param_name, param_value):
        """Setting the parameter to the ``self._interrnnt_params`` dictionary.
        ...
        self._interrnnt_params[param_name] = param_value

    def _verify_setup_was_called(self):
        """Can be used to verify if setup_interrnnt was called."""
        if not hasattr(self, '_interrnnt_params'):
            raise RuntimeError(
                'self.setup_interrnnt(rnnt_decoder_name, rnnt_loss_name, rnnt_wer_name) has to be '
                'called before InterRNNT loss can be used!'
            )

    def is_interrnnt_enabled(self) -> bool:
        """Returns whether interRNNT loss is enabled."""
        self._verify_setup_was_called()
        return self.get_interrnnt_param('enabled')

    def set_interrnnt_enabled(self, enabled: bool):
        """Can be used to enable/disable InterRNNT manually."""
        self._verify_setup_was_called()
        if enabled:  # checking if proper config parameters were specified
            if len(self.get_interrnnt_param('intermediate_loss_weights')) == 0:
                raise RuntimeError(
                    'InterRNNT cannot be enabled since interrnnt.loss_weights was not specified in the config.'
                )
            if len(self.get_interrnnt_param('apply_at_layers')) != len(
                self.get_interrnnt_param('intermediate_loss_weights')
            ):
                raise RuntimeError(
                    'InterRNNT cannot be enabled, since length of "loss_weights" does not match "apply_at_layers".'
                )
        assert enabled
        self.set_interrnnt_param('enabled', enabled)

    def finalize_interrnnt_metrics(self, metrics: Dict, outputs: List[Dict], prefix: str):
        """Finalizes InterRNNT WER and loss metrics for logging purposes.
        ...
        """
        if self.is_interrnnt_enabled():
            for layer_idx in self.get_interrnnt_param('apply_at_layers'):
                # assuming that if the first batch logged the metrics, then all batches did
                if f"{prefix}inter_rnnt_loss_l{layer_idx}" in outputs[0]:
                    loss = torch.stack([x[f"{prefix}inter_rnnt_loss_l{layer_idx}"] for x in outputs]).mean()
                    metrics["log"][f"{prefix}inter_rnnt_loss_l{layer_idx}"] = loss
    ...
    def get_captured_interrnnt_tensors(self) -> List[Tuple[torch.Tensor, torch.Tensor]]:
        """Returns a list of captured tensors from encoder: tuples of (output, length).

        Will additionally apply ``rnnt_decoder`` to the outputs.
        """
        if not self.is_interrnnt_enabled():
            return []
        ...
        for module_registry in AccessMixin.get_module_registry(self.encoder).values():
            for key in module_registry:
                if key.startswith("interrnnt/") and key in total_registry:
        ...
        # interrnnt/layer_output_X and interrnnt/layer_length_X tensors.
        # We need to apply decoder to each of them and compute RNNT loss.
        captured_tensors = []
        for layer_idx in self.get_interrnnt_param('apply_at_layers'):
            try:
                layer_outputs = total_registry[f"interrnnt/layer_output_{layer_idx}"]
                layer_lengths = total_registry[f"interrnnt/layer_length_{layer_idx}"]
            except KeyError:
                raise RuntimeError(
            ...
            if len(layer_outputs) > 1 or len(layer_lengths) > 1:
                raise RuntimeError(
                    "Make sure encoder.forward is called exactly one time before interRNNT loss is computed."
                )
...
#        inter_states,
        ...
        """Adding interRNNT losses if required.

        Will also register loss/wer metrics in the returned dictionary.

        Args:
            loss_value (torch.Tensor): regular loss tensor (will add interRNNT loss to it).
        ...
        """
        if not self.is_interrnnt_enabled() or not AccessMixin.is_access_enabled():
            print("HERE", self.is_interrnnt_enabled(), AccessMixin.is_access_enabled())
            return loss_value, {}
        metrics = {}
        if compute_loss:
        ...
        captured_tensors = self.get_captured_interrnnt_tensors()

        if compute_loss:
            loss_value *= self.get_interrnnt_param('main_loss_weight')

        for layer_idx, intermediate_result, loss_weight in zip(
            self.get_interrnnt_param('apply_at_layers'),
            captured_tensors,
            self.get_interrnnt_param('intermediate_loss_weights'),
        ):
...
#                print(
                ...
                metrics[f"{log_prefix}inter_rnnt_loss_l{layer_idx}"] = inter_loss_value.detach()
                loss_value += inter_loss_value * loss_weight
#            if compute_wer:
#                self.get_interrnnt_param('wer').update(
#                    predictions=intermediate_result[0],
#                    targets=transcript,
#                    targets_lengths=transcript_len,
#                    predictions_lengths=intermediate_result[1],
#                )
#                wer, wer_num, wer_denom = self.get_interrnnt_param('wer').compute()
#                self.get_interrnnt_param('wer').reset()
