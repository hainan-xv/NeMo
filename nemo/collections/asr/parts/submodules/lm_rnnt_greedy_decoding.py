...
class _GreedyLMRNNTInfer(Typing, ConfidenceMeasureMixin):
    ...
    def __init__(
        ...
        lm_model: rnnt_abstract.AbstractRNNTDecoder,
    ...
    def _lm_step(
        ...
        if isinstance(label, torch.Tensor):
            ...
            # Label is an integer
            if label == self._SOS:
                return self.lm.predict(None, hidden, add_sos=add_sos, batch_size=batch_size)

            label = label_collate([[label]])

        # output: [B, 1, K]
        return self.lm.predict(label, hidden, add_sos=add_sos, batch_size=batch_size)

    def _joint_step(self, enc, pred, lm, log_normalize: Optional[bool] = None):
        """
        ...
        """
        with torch.no_grad():
            logits = self.joint.joint(enc, pred, lm)
...
class GreedyLMRNNTInfer(_GreedyLMRNNTInfer):
    """A greedy transducer decoder.
...
class GreedyBatchedLMRNNTInfer(_GreedyLMRNNTInfer):
    ...
    def _greedy_decode_blank_as_pad(
        ...
        with torch.inference_mode():
            ...
            batchsize = x.shape[0]
            hypotheses = [
                rnnt_utils.Hypothesis(score=0.0, y_sequence=[], timestep=[], dec_state=None, lm_state=None) for _ in range(batchsize)
            ]
            ...
            for time_idx in range(max_out_len):
                ...
                while not_blank and (self.max_symbols is None or symbols_added < self.max_symbols):
                    ...
                    if time_idx == 0 and symbols_added == 0 and hidden is None:
                        ...
                        h, lm_hidden_prime = self._lm_step(self._SOS, hidden, batch_size=batchsize)
                    else:
                        ...
                        h, lm_hidden_prime = self._lm_step(last_label, lm_hidden, batch_size=batchsize)

                    # Batched joint step - Output = [B, V + 1]
                    # If preserving per-frame confidence, log_normalize must be true
                    logp = self._joint_step(f, g, h, log_normalize=True if self.preserve_frame_confidence else None)[
                        :, 0, 0, :
                    ]
                    ...
                    if all_blanks:
                        ...
                        if hidden is not None:
                            ...
                            lm_hidden_prime = self.lm.batch_copy_states(lm_hidden_prime, lm_hidden, blank_indices)
                            ...
                            lm_hidden_prime = self.lm.batch_copy_states(lm_hidden_prime, None, blank_indices, value=0.0)
        ...
        for batch_idx in range(batchsize):
            ...
            hypotheses[batch_idx].lm_state = self.lm.batch_select_state(lm_hidden, batch_idx)
...
class ExportedModelGreedyBatchedLMRNNTInfer:
...
class ONNXGreedyBatchedLMRNNTInfer(ExportedModelGreedyBatchedLMRNNTInfer):
...
class TorchscriptGreedyBatchedLMRNNTInfer(ExportedModelGreedyBatchedLMRNNTInfer):
    def __init__(
        self,
        encoder_model: str,
...
class GreedyMultiblankLMRNNTInfer(GreedyLMRNNTInfer):
...
class GreedyBatchedMultiblankLMRNNTInfer(GreedyBatchedLMRNNTInfer):
    """A batch level greedy transducer decoder.
    Batch level greedy decoding, performed auto-regressively.
    Args:
        decoder_model: rnnt_utils.AbstractRNNTDecoder implementation.
...
@dataclass
class GreedyBatchedLMRNNTInferConfig:
    max_symbols_per_step: Optional[int] = 10
    preserve_alignments: bool = False
    preserve_frame_confidence: bool = False
    confidence_measure_cfg: Optional[ConfidenceMeasureConfig] = ConfidenceMeasureConfig()
...
class GreedyTDTInfer(_GreedyLMRNNTInfer):
...
class GreedyBatchedTDTInfer(_GreedyLMRNNTInfer):
