...
class ConformerLayer(torch.nn.Module, AdapterModuleMixin, AccessMixin):
    ...
    def forward(self, x, att_mask=None, pos_emb=None, pad_mask=None, extra_indices=None, extra_att_mask=None, cache_last_channel=None, cache_last_time=None):
        """
        Args:
        ...
        if self.self_attention_model == 'rel_pos':
            x = self.self_attn(query=x, key=x, value=x, mask=att_mask, pos_emb=pos_emb, indices=extra_indices, att_mask=extra_att_mask, cache=cache_last_channel)
