...
__all__ = ['rnnt_loss', 'RNNTLossNumba', 'MultiblankRNNTLossNumba', 'LADRNNTLossNumba']
...
class _LADRNNTNumba(Function):
    ...
    def forward(
        ...
        if is_cuda:
            loss_func = rnnt.lad_rnnt_loss_gpu
        else:
            exit(-1)
...
def lad_rnnt_loss(
    ...
    """
    LAD RNN Transducer (https://arxiv.org/pdf/2211.03541.pdf) Loss (functional form)
    Args:
        acts: Tensor of (batch x seqLength x labelLength x outputDim) containing output from network
        labels: 2 dimensional Tensor containing all the targets of the batch with zero padded
        act_lens: Tensor of size (batch) containing size of each output sequence from the network
    ...
    return _LADRNNTNumba.apply(acts, labels, act_lens, label_lens, blank, durations, reduction, fastemit_lambda, clamp)
...
class LADRNNTLossNumba(Module):
    ...
    def __init__(
        ...
        super(LADRNNTLossNumba, self).__init__()
        self.blank = blank
        self.durations = durations
        self.fastemit_lambda = fastemit_lambda
        self.clamp = float(clamp) if clamp > 0 else 0.0
        self.reduction = reduction
        self.loss = _LADRNNTNumba.apply
        self.sigma = sigma
        self.omega = omega
