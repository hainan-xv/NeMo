...
def lad_rnnt_loss_gpu(
    ...
    lad_workspace = torch.zeros(len(durations), device=label_acts.device, dtype=torch.long, requires_grad=False)

    for i in range(0, len(durations)):
        lad_workspace[i] = durations[i]
    ...
    wrapper = gpu_rnnt.LADGPURNNT(
        minibatch=minibatch_size,
        maxT=maxT,
        maxU=maxU,
        alphabet_size=alphabet_size,
        workspace=gpu_workspace,
        lad_workspace=lad_workspace,
    ...
    del gpu_workspace, lad_workspace, wrapper
    return True
