...
@cuda.jit()
def multistream_compute_alphas_kernel(
    acts: torch.Tensor,
    denom: torch.Tensor,
    alphas: torch.Tensor,
        ...
        if u == 0:
            ...
            if t == 0:
                alphas[offset + u] = alphas[offset + u - 1] + logp(
                    denom, acts, maxT, maxU, alphabet_size, b, 0, u - 1, labels[u - 1, 0]
                )
                for k in range(1, maxK):
                    alphas[offset + u] += logp(
                        denom, acts, maxT, maxU, alphabet_size, b, 0, u - 1, labels[u - 1, k]
                    )
            ...
            # for t in range(1, T) for u in range(1, U) step to compute alphas[b, t, u]
                ...
                emit = alphas[offset + t * maxU + u - 1] + logp(
                    denom, acts, maxT, maxU, alphabet_size, b, t, u - 1, labels[u - 1, 0]
                )
                for k in range(1, maxK):
                    emit += logp(
                        denom, acts, maxT, maxU, alphabet_size, b, t, u - 1, labels[u - 1, k]
                    )
...
@cuda.jit()
def multistream_compute_betas_kernel(
    acts: torch.Tensor,
    denom: torch.Tensor,
    betas: torch.Tensor,
        ...
        if u == (U - 1):
            ...
            if t == T - 1:
                ...
                betas[offset + (T - 1) * maxU + u] = betas[offset + (T - 1) * maxU + u + 1] + logp(
                    denom, acts, maxT, maxU, alphabet_size, b, T - 1, u, labels[u, 0]
                )
                for k in range(1, maxK):
                    betas[offset + (T - 1) * maxU + u] += logp(
                        denom, acts, maxT, maxU, alphabet_size, b, T - 1, u, labels[u, k]
                    )
            elif (t >= 0) and (t < T - 1):
                ...
                emit = betas[offset + t * maxU + u + 1] + logp(
                    denom, acts, maxT, maxU, alphabet_size, b, t, u, labels[u, 0]
                )
                for k in range(1, maxK):
                    emit += logp(
                        denom, acts, maxT, maxU, alphabet_size, b, t, u, labels[u, k]
                    )
...
@cuda.jit()
def multistream_compute_grad_kernel(
    grads: torch.Tensor,
    acts: torch.Tensor,
    denom: torch.Tensor,
    alphas: torch.Tensor,
    ...
    if t < T and u < U:
        while idx < alphabet_size:
            ...
            grad = -999999  # Initialize to invalid value
            
            if (idx == blank_) and (t == T - 1) and (u == U - 1):
                grad = alphas[col] - logll[mb]
            elif (idx == blank_) and (t < T - 1):
                grad = alphas[col] + betas[col + maxU] - logll[mb]
            elif u < U - 1:
                tmp = 0.0
                found = False
                for k in range(maxK):
                    if idx == labels[u, k]:
                        tmp += alphas[col] + betas[col + 1] - logll[mb]
                        found = True
                    else:
                        tmp += acts[col * alphabet_size + labels[u, k]]
                if found:
                    grad = tmp
