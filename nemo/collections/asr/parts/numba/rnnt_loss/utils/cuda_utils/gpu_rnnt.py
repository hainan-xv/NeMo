...
class WordawareMultiblankGPURNNT(GPURNNT):
    def __init__(
        self,
        sigma: float,
        num_big_blanks: int,
    ...
    def compute_cost_and_score(
        ...
        # Compute alphas
        gpu_rnnt_kernel.compute_wordaware_multiblank_alphas_kernel[self.minibatch_, self.maxU_, self.stream_, 0](
            acts,
            denom,
            self.sigma,
            alphas,
        ...
        if training:
            # Compute betas
            gpu_rnnt_kernel.compute_wordaware_multiblank_betas_kernel[self.minibatch_, self.maxU_, self.stream_, 0](
                acts,
                denom,
                self.sigma,
                betas,
            ...
            grad_blocks_per_grid = self.minibatch_ * self.maxT_ * self.maxU_
            grad_threads_per_block = gpu_rnnt_kernel.GPU_RNNT_THREAD_SIZE
            gpu_rnnt_kernel.compute_wordaware_multiblank_grad_kernel[
                grad_blocks_per_grid, grad_threads_per_block, self.stream_, 0
            ](
                grads,
                acts,
