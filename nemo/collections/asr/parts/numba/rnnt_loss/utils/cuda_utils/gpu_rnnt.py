...
class LADGPURNNT(GPURNNT):
    def __init__(
        ...
        """
        Helper class to launch the CUDA Kernels to compute LAD Transducer Loss (https://arxiv.org/pdf/2211.03541).

        Args:
            sigma: Hyper-parameter related to the logit-normalization method in training lad transducers.
            ...
            workspace: An allocated chunk of memory that will be sliced off and reshaped into required
                blocks used as working memory.
            lad_workspace: An allocated chunk of memory that will be sliced off and reshaped into required
                blocks used as working memory specifically for the lad related computations.
            blank: Index of the RNNT blank token in the vocabulary. Generally the first or last token in the vocab.
            fastemit_lambda: Float scaling factor for FastEmit regularization. Refer to
                FastEmit: Low-latency Streaming ASR with Sequence-level Emission Regularization.
            clamp: Float value. When set to value >= 0.0, will clamp the gradient to [-clamp, clamp].
        ...
        self.lad_workspace = cuda.as_cuda_array(
            lad_workspace
        )  # a flat vector of integer numbers that represents allocated memory slices
    ...
    def compute_cost_and_score(
        ...
        if r < self.omega:
            ...
            # Compute alphas
            gpu_rnnt_kernel.compute_lad_alphas_kernel[self.minibatch_, self.maxU_, self.stream_, 0](
                label_acts,
                duration_acts,
                denom,
                self.sigma,
        ...
        if training:
            ...
            if r < self.omega:
                ...
                gpu_rnnt_kernel.compute_lad_betas_kernel[self.minibatch_, self.maxU_, self.stream_, 0](
                    label_acts,
                    duration_acts,
                    denom,
                    self.sigma,
                ...
                grad_blocks_per_grid = self.minibatch_ * self.maxT_ * self.maxU_
                grad_threads_per_block = gpu_rnnt_kernel.GPU_RNNT_THREAD_SIZE
                gpu_rnnt_kernel.compute_lad_grad_kernel[grad_blocks_per_grid, grad_threads_per_block, self.stream_, 0](
                    label_grads,
                    duration_grads,
                    label_acts,
                    duration_acts,
    ...
    def _prepare_workspace(self) -> (int, Tuple[torch.Tensor]):
        ...
        durations = self.lad_workspace[: self.num_durations]
