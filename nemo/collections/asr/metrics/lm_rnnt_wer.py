...
from nemo.collections.asr.parts.submodules import lm_rnnt_greedy_decoding as greedy_decode
from nemo.collections.asr.parts.utils.asr_confidence_utils import ConfidenceConfig, ConfidenceMixin
from nemo.collections.asr.parts.utils.rnnt_utils import Hypothesis, NBestHypotheses
from nemo.utils import logging

__all__ = ['LMRNNTDecoding', 'LMRNNTWER']


class AbstractLMRNNTDecoding(ConfidenceMixin):
    """
    ...
    def __init__(self, decoding_cfg, decoder, lm, joint, blank_id: int):
        super(AbstractLMRNNTDecoding, self).__init__()
        ...
        if self.cfg.strategy == 'greedy':
            if self.big_blank_durations is None:
                if self.durations is None:
                    self.decoding = greedy_decode.GreedyLMRNNTInfer(
                ...
                self.decoding = greedy_decode.GreedyMultiblankLMRNNTInfer(
                    decoder_model=decoder,
                    joint_model=joint,
                    blank_index=self.blank_id,
            ...
            if self.big_blank_durations is None:
                if self.durations is None:
                    self.decoding = greedy_decode.GreedyBatchedLMRNNTInfer(
                        decoder_model=decoder,
                        lm_model=lm,
                        joint_model=joint,
                        blank_index=self.blank_id,
                ...
                self.decoding = greedy_decode.GreedyBatchedMultiblankLMRNNTInfer(
                    decoder_model=decoder,
                    joint_model=joint,
                    blank_index=self.blank_id,
                    big_blank_durations=self.big_blank_durations,
            ...
            self.decoding = beam_decode.BeamLMRNNTInfer(
                decoder_model=decoder,
                joint_model=joint,
...
class LMRNNTDecoding(AbstractLMRNNTDecoding):
    """
    Used for performing RNN-T auto-regressive decoding of the Decoder+Joint network given the encoder state.
    ...
    def __init__(
        self, decoding_cfg, decoder, lm, joint, vocabulary,
    ):
        ...
        super(LMRNNTDecoding, self).__init__(
...
class LMRNNTWER(Metric):
    ...
    def __init__(
        self, decoding: LMRNNTDecoding, batch_dim_index=0, use_cer=False, log_prediction=True, dist_sync_on_step=False
    ):
        super(LMRNNTWER, self).__init__(dist_sync_on_step=dist_sync_on_step)
...
class LMRNNTDecodingConfig:
    ...
    greedy: greedy_decode.GreedyLMRNNTInferConfig = greedy_decode.GreedyLMRNNTInferConfig()

#    # beam decoding config
#    beam: beam_decode.BeamLMRNNTInferConfig = beam_decode.BeamLMRNNTInferConfig(beam_size=4)
