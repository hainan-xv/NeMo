...
from nemo.collections.asr.losses.rnnt_pytorch import LADRNNTLossPytorch
...
try:
    ...
    from nemo.collections.asr.parts.numba.rnnt_loss import LADRNNTLossNumba
...
RNNT_LOSS_RESOLVER = {
    ...
    "lad_rnnt_pytorch": RNNTLossConfig(
        loss_name="pytorch",
        lib_name="torch",
        min_version='0.0',
        is_available=True,
        installation_msg="Pure Pytorch implementation of LAD RNN-T loss. Slow and for debugging purposes only.",
    ),
}
...
def resolve_rnnt_loss(loss_name: str, blank_idx: int, loss_kwargs: dict = None) -> torch.nn.Module:
    ...
    elif loss_name == 'lad_rnnt_pytorch':
        durations = loss_kwargs.pop('durations', None)
        sigma = loss_kwargs.pop('sigma', 0.0)
        loss_func = LADRNNTLossPytorch(blank=blank_idx, durations=durations, reduction='none', sigma=sigma)
        _warn_unused_additional_kwargs(loss_name, loss_kwargs)
