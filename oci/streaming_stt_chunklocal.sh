...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-chunklocal
...
#
# CHUNK-LOCAL variant: identical to oci/streaming_stt_finetune_granary2_noblank_wer.sh
# except it runs OUR chunk-local recipe
#   /code/examples/speechlm2/conf/streaming_stt_granary2_lora_chunklocal.yaml
# which sets ++model.restrict_audio_cross_chunk=true. That restricts the LLM's
# attention so NO query (text OR audio) attends to a different chunk's audio
# frames: the LLM predicts each chunk's tokens from the text history plus only
# that chunk's audio -- p(text_k | text_<k, audio_k). The cache-aware encoder
# keeps its left context, so each chunk's frame embeddings still "know the past"
# acoustically; only the LLM's cross-chunk audio attention is removed. This is a
# strict superset of restrict_audio_to_own_chunk (see
# oci/streaming_stt_finetune_restricted.sh, which restricts text queries only).
#
# The recipe owns the modeling knobs (multi chunk-size list [2,4,7,10,14,28],
# att_context [70,1] with per-batch look-ahead = chunk-1, no-blank token, LoRA
# r=128, IdentityConnector, pre-aligned Granary 2.0 data, val_wer monitor). This
# launcher only passes training-loop overrides (lr/warmup/delay/seed/steps) plus
# infra -- it deliberately does NOT override chunk_size/att_context so the multi
# chunk-size schedule stands.
#
...
#   sbatch oci/streaming_stt_chunklocal.sh          # seed 42
#   sbatch oci/streaming_stt_chunklocal.sh 123      # seed 123
# ============================================================================
...
# Chunk-local attention: block ALL cross-chunk audio attention (text AND audio
# queries). Baked into the recipe; passed explicitly here so it is visible and
# toggleable.
RESTRICT_AUDIO_CROSS_CHUNK="${RESTRICT_AUDIO_CROSS_CHUNK:-true}"
...
# Config: OUR chunk-local recipe, shipped in the synced repo at /code.
CONFIG_PATH=/code/examples/speechlm2/conf/
CONFIG_NAME=streaming_stt_granary2_lora_chunklocal

EXP_NAME=granary2_chunklocal
...
&& echo "*** EXPERIMENT: chunk-local -> ++model.restrict_audio_cross_chunk=${RESTRICT_AUDIO_CROSS_CHUNK} ***" \
&& echo "*** (NO query attends to another chunk's audio; models p(text_k | text_<k, audio_k)) ***" \
...
&& python -c "import nemo.collections.speechlm2.models.streaming_stt_model as m; print('chunk-local supported:', 'restrict_audio_cross_chunk' in [f.name for f in __import__('dataclasses').fields(m.StreamingSTTModelConfig)])" \
&& pip show torch \
...
&& echo "Starting training (running MY code at /code, GRANARY 2.0 no-blank multi-chunk, CHUNK-LOCAL, val_wer)" \
&& python /code/examples/speechlm2/streaming_stt_train.py \
    ...
    ++model.restrict_audio_cross_chunk=${RESTRICT_AUDIO_CROSS_CHUNK} \
