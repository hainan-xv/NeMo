...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-g2-nb-wlen
...
#
# Mimics heh's oci_sslm_granary2_ft_lora_r1.sh (recipe
# streaming_stt_granary2_lora_mcs_noblank_v2) using OUR code + OUR recipe
#   /code/examples/speechlm2/conf/streaming_stt_granary2_lora_noblank_wer.yaml
# The ONLY intended difference from heh's setup is the checkpoint metric:
# we select on val_wer (min) instead of val_acc (max), because our code replaced
# teacher-forced val_acc with autoregressive-decode val_wer.
#
...
#   sbatch oci/streaming_stt_finetune_granary2_noblank_wer_wlendelay.sh          # seed 42
#   sbatch oci/streaming_stt_finetune_granary2_noblank_wer_wlendelay.sh 123      # seed 123
# ============================================================================
...
EXP_NAME=granary2_noblank_wer_wlendelay
...
&& echo "*** ALIGNMENT: word-length-dependent delay (<4L=3, 4L=2, 5L=1, >=6L=0 frames) -- fixed num_delay_frames ignored ***" \
...
&& python -c "import nemo.collections.speechlm2.models.streaming_stt_model as m; print('multi-chunk-size supported:', hasattr(m, '_repr_chunk_size'))" \
&& pip show torch \
