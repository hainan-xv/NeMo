...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-g2-nb-causalenc
...
#
# Same as streaming_stt_finetune_granary2_noblank_wer.sh (granary2, no-blank,
# multi chunk-size [2,4,7,10,14,28], val_wer), EXCEPT the encoder look-ahead is
# DECOUPLED from the LLM chunk size and FIXED to a strictly-causal setting:
#     att_context_size = [70, 0]   (left 70, right 0 -> 0 frame / 0 ms look-ahead)
#     couple_encoder_lookahead_to_chunk = false
# The LLM still trains on the variable per-batch chunk sizes (it aggregates a
# variable number of strictly-causal encoder frames per turn); only the encoder
# is pinned. Override ATT_CONTEXT to try a different fixed look-ahead
# (e.g. "[70,1]" = 80 ms, "[70,6]" = 480 ms).
#
# Usage (optional random seed as $1; defaults to 42):
#   cd /lustre/fsw/portfolios/llmservice/users/hainanx/NeMo79
#   sbatch oci/streaming_stt_finetune_granary2_noblank_wer_causalenc.sh          # seed 42
#   sbatch oci/streaming_stt_finetune_granary2_noblank_wer_causalenc.sh 123      # seed 123
# ============================================================================
...
# Fixed encoder attention context (decoupled from the LLM chunk size).
# [70, 0] = strictly causal (0 look-ahead). Override to sweep the latency.
ATT_CONTEXT="${ATT_CONTEXT:-[70,0]}"
...
EXP_NAME=granary2_noblank_wer_causalenc
...
&& echo "*** ENCODER: FIXED att_context_size=${ATT_CONTEXT} (decoupled from LLM chunk size) ***" \
...
&& python -c "from nemo.collections.speechlm2.models.streaming_stt_model import StreamingSTTModelConfig as C; print('fixed-encoder supported:', 'couple_encoder_lookahead_to_chunk' in C.__dataclass_fields__)" \
&& pip show torch \
...
&& echo "Starting training (running MY code at /code, GRANARY 2.0 no-blank multi-chunk, val_wer, FIXED strictly-causal encoder)" \
&& python /code/examples/speechlm2/streaming_stt_train.py \
    ...
    ++model.att_context_size=${ATT_CONTEXT} \
    ++model.couple_encoder_lookahead_to_chunk=false \
