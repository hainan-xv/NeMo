...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-chunkcompletion-promptctl-recover
...
#   recipe:     /code/examples/speechlm2/conf/streaming_stt_granary2_lora_chunkcompletion_promptctl_recover.yaml
#
# PROMPT-CONTROLLED + HISTORY-RECOVERY variant, built on the promptctl model.
# Three knobs (all set in the recipe):
#   1) EXACT DELAY (exact_delay=true, exact_max_delay=N): each batch samples an
#      integer delay ~ Uniform[0, N] frames and renders the exact number into the
#      prompt, so latency is controlled precisely at inference.
#   2) TEXT REPRESENTATION (vary_text_repr=true): each batch samples one of the 4
#      capitalization x punctuation combos, states it in the prompt, and transforms
#      the chunk target text accordingly.
#   3) HISTORY-WORD RECOVERY (history_word_recovery_prob=0.3, audio_history_chunks=0):
#      an eligible chunk drops the previous chunk's last word from its history and
#      must recover it from the current chunk's audio (encoder left-context) + text
#      history -- NO prior-chunk audio window ("not the multiple-chunk thing").
# Otherwise identical to the chunk-completion objective (packed spine+branch,
# single O(L) forward, streaming spine-KV decode).
#
...
#   sbatch oci/chunk_completion_promptctl_recover.sh          # seed 42
#   sbatch oci/chunk_completion_promptctl_recover.sh 123      # seed 123
# ============================================================================
...
CONFIG_NAME=streaming_stt_granary2_lora_chunkcompletion_promptctl_recover

EXP_NAME=granary2_chunkcompletion_promptctl_recover
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (chunk-completion, EXACT-delay + TEXT-repr + HISTORY-RECOVERY[no window], granary2, no-blank) ***" \
&& echo "*** OBJECTIVE: p(words_k | text_history_<k, audio_k); packed spine+branch, exact delay + cap/punct + drop-and-recover history word ***" \
