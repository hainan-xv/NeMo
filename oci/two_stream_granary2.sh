...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-2stream-g2
...
#
# TWO-STREAM on GRANARY 2.0 data. Based on oci/baseline_granary2.sh (Granary 2.0
# pre-aligned manifest + compact template), plus:
#   ++model.two_stream_last_layer=true   (text runs layers[:-1] alone; audio is
#                                         fused only in the FINAL layer)
# LoRA (from the recipe) is applied to all layers, and the speech encoder trains
# fully -- this is the two-stream analogue of the baseline.
#
...
#   sbatch oci/two_stream_granary2.sh
# ============================================================================
...
&& python -c "import nemo.collections.speechlm2.models.streaming_stt_model as m; print('two-stream supported:', 'two_stream_llm_forward' in dir(m))" \
&& pip show torch \
...
&& echo "Starting training (running MY code at /code, GRANARY 2.0 data, TWO-STREAM)" \
