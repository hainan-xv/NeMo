...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-chunkcompletion-hist1
...
#   recipe:     /code/examples/speechlm2/conf/streaming_stt_granary2_lora_chunkcompletion.yaml
#
# Instead of one interleaved audio-text-audio-text causal stream, each utterance
# is packed as a pure-text spine (the transcript history) plus one branch per
# chunk (its audio wrapped in <|vision_start|>/<|vision_end|> + the words it
# reveals). The model is trained to extract the words carried by each new audio
# chunk given the transcript so far -- p(words_k | text_history_<k, audio_k) --
# in a single O(L) forward with a custom 4D mask + position_ids. Inference is a
# streaming loop with a plain-text spine KV cache; each chunk's audio is evicted
# after its words are decoded.
#
...
#   sbatch oci/chunk_completion.sh          # seed 42
#   sbatch oci/chunk_completion.sh 123      # seed 123
# ============================================================================
...
CONFIG_NAME=streaming_stt_granary2_lora_chunkcompletion_hist1

EXP_NAME=granary2_chunkcompletion_hist1
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (chunk-completion, granary2, no-blank, multi chunk-size [2,4,7,10,14,28]) ***" \
