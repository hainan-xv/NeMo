...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-2stream-llf
...
#
# TWO-STREAM + FULL last-layer training (NO LoRA). Based on the older baseline
# (oci/streaming_stt_finetune.sh) data (recipe's online-alignment data, NOT the
# Granary 2.0 pre-aligned manifest), plus:
#   ++model.two_stream_last_layer=true       (text runs layers[:-1] alone; audio
#                                             is fused only in the FINAL layer)
#   ~model.lora                               (disable LoRA entirely)
#   model.freeze_llm_model=true               (freeze the whole LLM body...)
#   ++model.unfreeze_last_n_llm_layers=1      (...then unfreeze ONLY the final
#                                             decoder layer for full-weight
#                                             fine-tuning)
#   model.freeze_speech_encoder=true          (freeze the whole speech encoder...)
#   ++model.unfreeze_last_n_encoder_layers=1  (...then unfreeze ONLY the final
#                                             Conformer layer)
# So only the TOP layer of both the encoder and the LLM trains (full-rank),
# plus the modality adapter/projection. Compare against the LoRA variants.
#
...
#   sbatch oci/streaming_stt_finetune_two_stream_lastlayer_full.sh
# ============================================================================
...
# Freeze the whole speech encoder, then unfreeze only the last N Conformer
# layers below (symmetric with the LLM: train just the top of each stack).
FREEZE_SPEECH_ENCODER=true

LR=0.0004
WARMUP_STEPS=15000

# Number of top LLM decoder layers to unfreeze for full-weight training (the
# rest of the LLM body stays frozen). 1 = only the two-stream fusion layer.
UNFREEZE_LAST_N="${UNFREEZE_LAST_N:-1}"
# Number of top encoder (Conformer) layers to unfreeze (rest of encoder frozen).
UNFREEZE_LAST_N_ENC="${UNFREEZE_LAST_N_ENC:-1}"
...
EXP_NAME=two_stream_lastlayer_full
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** EXPERIMENT: two_stream + FULL top-layer training (no LoRA) ***" \
&& echo "***   ++model.two_stream_last_layer=true ***" \
&& echo "***   ~model.lora  model.freeze_llm_model=true  ++model.unfreeze_last_n_llm_layers=${UNFREEZE_LAST_N} ***" \
&& echo "***   model.freeze_speech_encoder=true  ++model.unfreeze_last_n_encoder_layers=${UNFREEZE_LAST_N_ENC} ***" \
&& echo "***   (only the TOP layer of encoder+LLM trains, full-rank; lower layers frozen) ***" \
&& nvidia-smi \
...
&& python -c "import inspect, nemo.collections.speechlm2.models.streaming_stt_model as m; print('two-stream supported:', 'two_stream_llm_forward' in dir(m)); print('unfreeze-last-n supported:', 'unfreeze_last_n_llm_layers' in inspect.getsource(m.StreamingSTTModel._apply_freeze_config))" \
&& pip show torch \
...
&& python /code/examples/speechlm2/streaming_stt_train.py \
    ...
    ++model.unfreeze_last_n_llm_layers=${UNFREEZE_LAST_N} \
    ++model.unfreeze_last_n_encoder_layers=${UNFREEZE_LAST_N_ENC} \
