...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-2slf-g2
...
#
# TWO-STREAM + FULL top-layer training (NO LoRA) for BOTH the LLM and the speech
# encoder, on GRANARY 2.0 data. Based on oci/baseline_granary2.sh, plus:
#   ++model.two_stream_last_layer=true       (audio fused only in the FINAL layer)
#   ~model.lora                              (disable LoRA entirely)
#   model.freeze_llm_model=true              (freeze LLM body...)
#   ++model.unfreeze_last_n_llm_layers=1     (...unfreeze ONLY the final LLM layer)
#   model.freeze_speech_encoder=true         (freeze encoder...)
#   ++model.unfreeze_last_n_encoder_layers=1 (...unfreeze ONLY the final Conformer layer)
# So only the TOP layer of each stack trains (full-rank), plus the modality
# adapter/projection.
#
...
#   sbatch oci/two_stream_lastlayer_full_granary2.sh
# ============================================================================
...
EXP_NAME=two_stream_lastlayer_full_granary2
...
&& echo "*** EXPERIMENT: two_stream + FULL top-layer training (no LoRA) for LLM+encoder ***" \
&& echo "***   ++model.two_stream_last_layer=true  ~model.lora ***" \
&& echo "***   freeze_llm_model=true  unfreeze_last_n_llm_layers=${UNFREEZE_LAST_N} ***" \
&& echo "***   freeze_speech_encoder=true  unfreeze_last_n_encoder_layers=${UNFREEZE_LAST_N_ENC} ***" \
...
&& python -c "import inspect, nemo.collections.speechlm2.models.streaming_stt_model as m; print('two-stream supported:', 'two_stream_llm_forward' in dir(m)); print('unfreeze-last-n supported:', 'unfreeze_last_n_encoder_layers' in inspect.getsource(m.StreamingSTTModel._apply_freeze_config))" \
&& pip show torch \
...
&& echo "Starting training (running MY code at /code, GRANARY 2.0, TWO-STREAM + full top-layer)" \
