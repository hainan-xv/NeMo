...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-baseline-g2
...
#
# GRANARY 2.0 DATA variant: identical to oci/streaming_stt_finetune.sh
# (baseline) except it trains on the pre-aligned Granary v2.0 manifest
# (alignments embedded in the cuts, so the online Qwen forced aligner is
# removed -> faster, no on-the-fly alignment). Override TRAIN_INPUT_CFG to
# point at a different manifest, or set it empty to fall back to the recipe's
# default online-alignment data.
#
...
#   sbatch oci/baseline_granary2.sh
# ============================================================================
...
&& echo "*** (online forced aligner removed) ***" \
&& nvidia-smi \
...
&& echo "Starting training (running MY code at /code, GRANARY 2.0 data)" \
