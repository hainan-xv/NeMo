...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-g2-nb-2s-nf4
...
#
# TWO-STREAM variant of streaming_stt_finetune_granary2_noblank_wer.sh.
# Same setup as heh's granary2 (multi chunk-size, no-blank, LoRA r128 on ALL LLM
# layers, fully-trainable encoder, val_wer) PLUS our two-stream last-layer fusion:
#   ++model.two_stream_last_layer=true
# The text stream runs through layers[:-1] alone; the raw audio-encoder frames
# are interleaved back only in the FINAL layer for cross-modal fusion. LoRA (all
# layers) and the encoder train exactly as in the baseline; no code change needed
# (the two-stream + multi-chunk paths are already supported).
#
...
# r=128, IdentityConnector, pre-aligned Granary 2.0 data). This launcher only
# passes the same training-loop overrides heh used (lr/warmup/delay/seed/steps)
# plus infra — it deliberately does NOT override chunk_size/att_context so the
# multi chunk-size schedule stands.
#
# Infra is our known-good baseline_granary2.sh setup (token-file secrets, our
# NeMo79 at /code, results on the nemotron project, HEH_DIR + DONGJI_DIR mounts).
#
# Usage (optional random seed as $1; defaults to 42):
#   cd /lustre/fsw/portfolios/llmservice/users/hainanx/NeMo79
#   sbatch oci/streaming_stt_finetune_granary2_noblank_wer_twostream_nf4.sh          # seed 42
#   sbatch oci/streaming_stt_finetune_granary2_noblank_wer_twostream_nf4.sh 123      # seed 123
# ============================================================================
...
EXP_NAME=granary2_noblank_wer_twostream_nf4
...
&& echo "*** EXPERIMENT: two_stream fusion in the top 4 layers (interleave at layer n-4); LoRA all layers; encoder trainable ***" \
...
&& python -c "import nemo.collections.speechlm2.models.streaming_stt_model as m; print('multi-chunk-size supported:', hasattr(m, '_repr_chunk_size')); print('two-stream supported:', 'two_stream_llm_forward' in dir(m))" \
&& pip show torch \
...
&& echo "Starting training (running MY code at /code, GRANARY 2.0 no-blank multi-chunk, val_wer)" \
&& python /code/examples/speechlm2/streaming_stt_train.py \
    ...
    ++model.two_stream_last_layer=true \
    ++model.two_stream_num_fusion_layers=4 \
