...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-restrictattn
...
#
# RESTRICTED-ATTENTION variant: identical to oci/streaming_stt_finetune.sh
# (baseline) except it enables ++model.restrict_audio_to_own_chunk=true, so a
# text (transcription) query attends to all text + only its own chunk's audio
# frames (audio queries stay causal). Use this for the A/B vs baseline.
#
...
#   sbatch oci/streaming_stt_finetune_restricted.sh
# ============================================================================
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** EXPERIMENT: restricted_attention -> ++model.restrict_audio_to_own_chunk=true ***" \
&& echo "*** (text queries attend to all text + only their own chunk's audio) ***" \
&& nvidia-smi \
...
&& python -c "import inspect, nemo.collections.speechlm2.models.streaming_stt_model as m; print('restricted-attention supported:', 'build_chunk_restricted_mask' in dir(m))" \
&& pip show torch \
...
&& echo "Starting training (running MY code at /code, RESTRICTED ATTENTION)" \
