...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-chunkcompletion-promptctl-all
...
#
# UNIFIED PROMPT-CONTROLLED variant: ONE model whose three streaming knobs are all
# stated explicitly in the prompt, each drawn per batch:
#   entrypoint: examples/speechlm2/chunk_completion_train.py
#               (ChunkCompletionSTTModel + ChunkCompletionSTTDataset)
#   recipe:     /code/examples/speechlm2/conf/streaming_stt_granary2_lora_chunkcompletion_promptctl_all.yaml
#
#   1) CHUNK SIZE  -- the per-batch chunk size (frames) is rendered into the prompt.
#   2) EXACT DELAY -- delay ~ Uniform[0,4] frames per batch, rendered into the prompt.
#   3) SELF-CORRECTION on/off -- FORCED/DAgger objective toggled per batch
#      (self_correction_batch_prob=0.5); the prompt states whether the model MAY
#      correct (delete + rewrite a mis-committed word) or must NOT. Each branch sees
#      2 chunks of audio (audio_history_chunks=1) so the mis-committed word's own
#      audio is in-window.
#   (also inherits the cap/punct text-representation prompt knob from _promptctl.)
#
# Built on _promptctl and INITIALIZED from its checkpoint (set INIT_CKPT below), so
# this run only has to learn the two new prompt clauses (chunk size + correction).
#
...
#   sbatch oci/chunk_completion_promptctl_all.sh          # seed 42
#   sbatch oci/chunk_completion_promptctl_all.sh 123      # seed 123
#
...
CONFIG_NAME=streaming_stt_granary2_lora_chunkcompletion_promptctl_all

EXP_NAME=granary2_chunkcompletion_promptctl_all
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (chunk-completion, UNIFIED prompt-ctl: chunk-size + exact-delay + self-correction on/off, forced/DAgger + 2-chunk window, init=${INIT_CKPT:-none}, granary2, no-blank) ***" \
&& echo "*** OBJECTIVE: p(words_k | text_history_<k, audio_k); packed spine+branch, chunk-size + delay + correction + cap/punct via prompt ***" \
