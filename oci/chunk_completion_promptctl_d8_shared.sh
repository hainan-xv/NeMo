...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-chunkcompletion-promptctl-d8-shared
...
#   recipe:     /code/examples/speechlm2/conf/streaming_stt_granary2_lora_chunkcompletion_promptctl_d8_shared.yaml
#   (max-delay-8 + FIXED 14-frame audio window via a SHARED audio track:
#    exact_max_delay=8, audio_window_frames=14, shared_audio_track=true. The encoder
#    frames are laid down once and each branch is windowed via the mask -> packed
#    length independent of the window, so the full chunk_size list is kept.)
#
# PROMPT-CONTROLLED variant, built on the multi-delay-prompt model. Two batch-
# shared, prompt-controlled knobs (set in the recipe):
...
#   sbatch oci/chunk_completion_promptctl_d8_shared.sh          # seed 42
#   sbatch oci/chunk_completion_promptctl_d8_shared.sh 123      # seed 123
# ============================================================================
...
CONFIG_NAME=streaming_stt_granary2_lora_chunkcompletion_promptctl_d8_shared

EXP_NAME=granary2_chunkcompletion_promptctl_d8_shared
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (chunk-completion, EXACT-delay[0..8] + TEXT-repr + SHARED-AUDIO fixed 14-frame window, granary2, no-blank) ***" \
