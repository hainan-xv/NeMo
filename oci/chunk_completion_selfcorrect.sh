...
#
# Set INIT_CKPT to the latest _promptctl checkpoint to initialize weights from it:
#   INIT_CKPT=/results/.../checkpoints/xxx.ckpt sbatch oci/chunk_completion_selfcorrect.sh
# ============================================================================
...
# Initialize weights from the latest _promptctl checkpoint (weights + optimizer via
# resume). Point this at the promptctl run's .ckpt. Leave empty to train from the
# base pretrained LLM+ASR instead.
INIT_CKPT="${INIT_CKPT:-}"
INIT_CKPT_ARG=""
if [[ -n "$INIT_CKPT" ]]; then
