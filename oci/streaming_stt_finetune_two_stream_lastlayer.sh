...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-2stream-lll
...
#
# TWO-STREAM + LAST-LAYER-ONLY LoRA variant. Based on the older baseline
...
#                                          decoder layer -- where all cross-modal
#                                          fusion happens; lower text-only layers
#                                          stay frozen base weights)
# LoRA target_modules are widened for that single layer to compensate for the
# reduced breadth. Use this for the A/B vs the full-LoRA two-stream run.
#
...
#   sbatch oci/streaming_stt_finetune_two_stream_lastlayer.sh
# ============================================================================
...
EXP_NAME=two_stream_lastlayer_lora
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** EXPERIMENT: two_stream + last-layer-only LoRA ***" \
&& echo "***   ++model.two_stream_last_layer=true ***" \
&& echo "***   ++model.lora.last_layer_only=true  target_modules=${LORA_TARGET_MODULES} ***" \
&& echo "***   (LoRA only on the final fusion layer; lower text layers frozen) ***" \
&& nvidia-smi \
...
&& echo "Starting training (running MY code at /code, TWO-STREAM + last-layer LoRA)" \
