...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-chunkcompletion-selfcorrect-prefix-hist1
...
#   recipe:     /code/examples/speechlm2/conf/streaming_stt_granary2_lora_chunkcompletion_selfcorrect_prefix_hist1.yaml
#   (2-chunk-window variant: audio_history_chunks=1, so each branch conditions on the
#    previous + current chunk's audio -- the corrected word's own audio is in-window.)
#
...
#   sbatch oci/chunk_completion_selfcorrect_prefix_hist1.sh          # seed 42
#   sbatch oci/chunk_completion_selfcorrect_prefix_hist1.sh 123      # seed 123
#
...
CONFIG_NAME=streaming_stt_granary2_lora_chunkcompletion_selfcorrect_prefix_hist1

EXP_NAME=granary2_chunkcompletion_selfcorrect_prefix_hist1
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (chunk-completion, SELF-CORRECTION prefix-heuristic + 2-chunk window, init=${INIT_CKPT:-none}, granary2, no-blank) ***" \
