...
# FastConformer-TDT large model (~120M, 8x dw-striding, conv_kernel 9) on
...
#
# Goal: quickly compare unitary-residual vs standard identity-residual encoders
# on a small local dataset.
#
# Usage:
#   ./train_librispeech.sh [USE_UNITARY_RESIDUAL] [GPU_ID]
#     arg1 USE_UNITARY_RESIDUAL : true (default) | false  -> unitary vs standard baseline
#     arg2 GPU_ID               : CUDA device id to use (default 0)
#   e.g.  ./train_librispeech.sh true 0     # unitary run on GPU 0
#         ./train_librispeech.sh false 1    # standard baseline on GPU 1
#
...
#
# Entrypoint:  examples/asr/asr_transducer/speech_to_text_rnnt_bpe.py
# Base config: examples/asr/conf/fastconformer/fast-conformer_tdt_bpe.yaml (FastConformer-large)
# ============================================================================
...
# FastConformer-large (TDT): 17 layers, d_model 512, 8 heads, conv kernel 9, 8x
...
if [ "${USE_UNITARY_RESIDUAL}" = "true" ]; then
  ...
  MATCH_OUT=$(python - "${D_MODEL}" "${N_LAYERS}" "${N_HEADS}" "${FF_EXPANSION_FACTOR}" "${CONV_KERNEL_SIZE}" "${SUBSAMPLING_FACTOR}" "${TOKENIZER_DIR}" "${REPO_ROOT}/examples/asr/conf/fastconformer/fast-conformer_tdt_bpe.yaml" rnnt <<'PY'
import sys
...
EXP_NAME="ls960_fastconformerLargeTdt_${RESIDUAL_TAG}_L${N_LAYERS}_d${D_MODEL_EFFECTIVE}_h${N_HEADS}_v${VOCAB_SIZE}"
...
# --- Build the BPE tokenizer once (idempotent) ---
...
python "${REPO_ROOT}/examples/asr/asr_transducer/speech_to_text_rnnt_bpe.py" \
    --config-path="${REPO_ROOT}/examples/asr/conf/fastconformer" \
    --config-name=fast-conformer_tdt_bpe \
    name="${EXP_NAME}" \
