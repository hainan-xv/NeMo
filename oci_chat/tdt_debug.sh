...
#SBATCH -J nemotron_speechprod_asr:tdt-debug-toklog-g2
...
# ============================================================================
# DEBUG launcher: same parakeet-tdt-0.6b-v2 setup on Granary v2.0, but with
# training-time TOKENIZATION LOGGING enabled to inspect how transcripts are
# tokenized (original text -> subword pieces -> token ids -> #<unk>). Use this to
# check how often / on which characters the transcripts fall back to <unk>.
#
...
#   grep -E 'tok-debug|<unk>' <results>/slurm-*.out
#
# Submit from an OCI login node:
#   ./sync_to_oci.sh
#   sbatch oci_chat/tdt_debug.sh
#   NEMO_LOG_TOKENIZATION=500 sbatch oci_chat/tdt_debug.sh   # log more samples
#
# ---------------------------------------------------------------------------
...
# DEBUG: log the first N transcripts' tokenization (text -> pieces -> ids -> #<unk>)
# from the training dataloader (rank 0). 0 disables. See
# nemo/collections/asr/data/audio_to_text_lhotse.py (_debug_log_tokenization).
NEMO_LOG_TOKENIZATION="${NEMO_LOG_TOKENIZATION:-200}"
...
EXP_NAME="${EXP_NAME:-${CLUSTER}_tdt_debug_toklog_parakeet06bv2_g2_lr${LR}_${PRECISION}_n${SLURM_JOB_NUM_NODES}}"
...
&& echo "*** DEBUG: NEMO_LOG_TOKENIZATION=${NEMO_LOG_TOKENIZATION} (grep 'tok-debug' for text->pieces->ids->#unk, rank0) ***" \
...
&& export NEMO_LOG_TOKENIZATION=${NEMO_LOG_TOKENIZATION} \
