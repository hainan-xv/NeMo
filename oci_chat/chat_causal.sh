...
#SBATCH -J nemotron_speechprod_asr:chat-causal-scratch-rnnt-g2
...
# ============================================================================
# OCI launcher for the STRICTLY-CAUSAL CHAT (Chunk-wise Attention Transducer).
#
# Fully causal (offline-free / streaming with zero lookahead) variant of
# oci_chat/chat.sh:
#   - att_context_size    = [64,0]   (64 left, 0 right -> no lookahead)
#   - att_context_style   = chunked_limited (cache-aware; [64,0] is causal)
#   - causal_downsampling = true     (causal subsampling)
#   - conv_context_size   = causal   (causal convolution)
#   - joint.chunk_size    = ${CHAT_CHUNK_SIZE} (fixed 14; set explicitly because
#                           right_context=0 would otherwise infer chunk_size=1)
#   - encoder trained FROM SCRATCH (INIT_ENCODER_MODE=scratch, no warm-start)
#
# Model: Cache-Aware FastConformer RNNT with the cross-attention joint
# (RNNTAttJoint). Same config file as chat.sh (the causal knobs are overridden
# below):
...
#   ./sync_to_oci.sh
#   sbatch oci_chat/chat_causal.sh
# ============================================================================
...
# Strictly-causal encoder knobs (override the config defaults).
ATT_CONTEXT="${ATT_CONTEXT:-[64,0]}"                     # 64 left, 0 right -> no lookahead
ATT_CONTEXT_STYLE="${ATT_CONTEXT_STYLE:-chunked_limited}" # [64,0] satisfies left % (right+1) == 0
CAUSAL_DOWNSAMPLING="${CAUSAL_DOWNSAMPLING:-true}"        # causal subsampling
CONV_CONTEXT_SIZE="${CONV_CONTEXT_SIZE:-causal}"          # causal convolution
CTX_TAG="$(echo "${ATT_CONTEXT}" | tr -d '[] ' | tr ',' '_')"

# CHAT joint chunk size. With right_context=0 it cannot be inferred, so it is
...
# Train the encoder FROM SCRATCH (no warm-start). Override with INIT_ENCODER_MODE
# to warm-start from a checkpoint if desired (local .nemo | hf).
...
EXP_NAME="${EXP_NAME:-${CLUSTER}_chat_causal_scratch_rnnt_g2_ctx${CTX_TAG}_chunk${CHAT_CHUNK_SIZE}_lr${LR}_n${SLURM_JOB_NUM_NODES}}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING STRICTLY-CAUSAL CHAT (FastConformer RNNT) - Granary 2.0********" \
&& echo "*** CONFIG: ${CONFIG_NAME} | ATT_CONTEXT=${ATT_CONTEXT} style=${ATT_CONTEXT_STYLE} ***" \
&& echo "*** CAUSAL: causal_downsampling=${CAUSAL_DOWNSAMPLING} conv_context_size=${CONV_CONTEXT_SIZE} | CHAT chunk_size=${CHAT_CHUNK_SIZE} ***" \
...
&& echo "Starting training (running MY code at /code, strictly-causal CHAT model, GRANARY 2.0 data)" \
