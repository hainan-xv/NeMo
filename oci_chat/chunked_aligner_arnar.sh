...
# ============================================================================
# HAINAN (stochastic AR/NAR) variant of the streaming Chunked-Aligner.
#
...
# output position u, with probability MASKING_PROB (default 0.5). Masked positions
# must be predicted from the encoder alone (NAR); unmasked positions use the AR
# history, so the model trains to work both autoregressively and
# non-autoregressively. Inference is unchanged (standard AR chunked greedy decode).
#
# Override the masking probability with e.g.: MASKING_PROB=0.3 sbatch oci_chat/chunked_aligner_arnar.sh
#
# ---------------------------------------------------------------------------
...
EXP_NAME="${EXP_NAME:-${CLUSTER}_chunked_aligner_arnar_parakeet_g2_chunk${CHUNK_SIZE}${MASK_TAG}_lr${LR}_${PRECISION}_n${SLURM_JOB_NUM_NODES}}"
