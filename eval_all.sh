...
for d in "$ROOT"/*/; do
    ...
    for c in $CHUNK_LIST; do
        ...
        # Layout:
        #   <exp>/eval_<ckpt_ts>/chunk<C>[_extras]/
        # Two globs, not one "chunk${c}*": an exact match plus a "_"-suffixed one,
        # so chunk 2 cannot match chunk 28's directory.
        for ed in "$d"eval_*/chunk"${c}"/ "$d"eval_*/chunk"${c}"_*/; do
            [ -d "$ed" ] || continue
...
if [[ "$DRY_RUN" -eq 0 ]]; then
    ...
    echo "Results: ${RESULTS_ROOT}/<exp>/eval_<ckpt_ts>/<decode_config>/aggregate.log"
fi
