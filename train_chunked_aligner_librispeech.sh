...
# CHUNKED-ALIGNER ASR model on LibriSpeech-960, with a standard Conformer encoder.
#
# The Chunked Aligner makes the offline Aligner-Encoder streaming-friendly by
# constraining alignment to fixed-size encoder chunks and learning the chunk-level
# alignment from scratch with a full-sum (forward-backward) objective (like
# RNN-T/CTC). It reuses an RNN-T prediction net + joint; within a chunk the
# diagonal advances one frame per emitted token and the blank doubles as an
# end-of-chunk (EOC) signal. See
# examples/asr/conf/aligner/chunked_aligner_encoder_bpe.yaml.
#
# Usage:
#   ./train_chunked_aligner_librispeech.sh [CHUNK_SIZE] [GPU_ID]
#     arg1 CHUNK_SIZE : encoder frames per chunk (default 12)
#     arg2 GPU_ID     : CUDA device id to use (default 0)
#   e.g.  ./train_chunked_aligner_librispeech.sh 12 0
#         ./train_chunked_aligner_librispeech.sh 16 1
#
...
#
# Entrypoint:  examples/asr/asr_aligner/speech_to_text_chunked_aligner_bpe.py
# Base config: examples/asr/conf/aligner/chunked_aligner_encoder_bpe.yaml (standard Conformer)
# ============================================================================
...
EXP_NAME="ls960_chunkedAligner_C${CHUNK_SIZE}_conformerLarge_L${N_LAYERS}_d${D_MODEL}_h${N_HEADS}_v${VOCAB_SIZE}"
...
python "${REPO_ROOT}/examples/asr/asr_aligner/speech_to_text_chunked_aligner_bpe.py" \
    --config-path="${REPO_ROOT}/examples/asr/conf/aligner" \
    --config-name=chunked_aligner_encoder_bpe \
    name="${EXP_NAME}" \
