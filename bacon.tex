...
In all result tables, $p$ denotes the two-sided significance of the BACON-vs.-baseline comparison, computed with a paired bootstrap test over the per-utterance outputs; we treat $p<0.05$ as significant.
...
across conditions, with the larger gains on \texttt{test-other}.}
\label{tab:libri_wer}
\begin{tabular}{llccc}
\toprule
...
with \textsc{must-c} gains of +0.71 and +1.01, respectively. All four improvements are statistically significant (paired bootstrap, $p<0.01$).
...
half of the channels causal. This comparison directly isolates the contribution
of BACON's dual-mode (split) design against the fully boundary-aware variant.
