...
"""Train the SCRIPT streaming SpeechLM.

Same entrypoint shape as ``streaming_stt_train.py`` but uses
:class:`ScriptSTTModel` + :class:`ScriptSTTDataset`, which frame
streaming ASR as ``p(words_k | text_history_<k, audio_k)`` via a packed
spine+branch sequence (see nemo/collections/speechlm2/parts/script.py).
"""
import os
...
def train(cfg):
    ...
    dataset = ScriptSTTDataset(cfg=dataset_cfg, tokenizer=model.tokenizer, defer_get_batch=defer_get_batch)
