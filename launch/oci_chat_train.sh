...
#
# OCI-IAD PORT of dfw_chat_train.sh. Same knobs, same hydra overrides, same
# semantics -- only the cluster-specific paths differ (portfolio roots, the
# granary input_cfg variant, and the results/wandb namespace). Kept as a
# separate file rather than merged into chat_train.sh so the 68 existing OCI
# launchers keep the trainer they were validated against.
# ============================================================================
set -uo pipefail
...
GPUS_PER_NODE=8
# OCI-IAD layout. Unlike DFW, hainanx's tree and heh's tree live in
# DIFFERENT portfolios, so these cannot be derived from one root.
LUSTRE=/lustre/fsw/portfolios/nemotron
HEH=/lustre/fsw/portfolios/llmservice/users/heh
MYDIR=${LUSTRE}/users/hainanx
...
EXP_NAME="${EXP_NAME:-oci_granary2_chat_${LOSS_TYPE}}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** OCI CHAT transducer (RNNTAttJoint), loss_type=${LOSS_TYPE} ***" \
