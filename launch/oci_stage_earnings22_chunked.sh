...
#SBATCH -J nemotron_speechprod_asr:stage-earnings22-chunked
...
# ============================================================================
# Stage ONLY the ArtificialAnalysis chunked Earnings22 into the OCI leaderboard
# cache, so the OCI eval suite matches DFW's macro-7.
#
#   sbatch launch/oci_stage_earnings22_chunked.sh
#
# WHY. DFW's macro-7 scores earnings22_cleaned_aa_chunked and explicitly EXCLUDES
# plain earnings22; the OCI cache only has the plain one. That single slot is the
# whole difference between the two suites, and it is not a small one -- plain
# earnings22 scored 11.76 for the SCRIPT arm where the chunked variant runs ~8.8
# on comparable DFW arms. Until this is staged, OCI and DFW numbers differ for a
# reason that has nothing to do with the models.
#
# Writes into ${CACHE_DIR}/earnings22_cleaned_aa_chunked/test/. That path is under
# the llmservice PROJECT quota (2.9 T of 100 T), not the 10 G per-user quota on
# /lustre/fsw, so size is not a constraint here.
# ============================================================================
