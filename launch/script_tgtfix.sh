...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-tgtfix
...
# ============================================================================
# SCRIPT with the alignment/target fix -- the A/B partner of granary2_script_baseline.
#
#   sbatch launch/script_tgtfix.sh          <- no arguments
#
# ONE VARIABLE. This is launch/script_baseline.sh with two changes: CONFIG_NAME
# and EXP_NAME. The recipe is streaming_stt_granary2_lora_script.yaml copied
# verbatim plus model.respell_targets / data.dataset.respell_targets. Same
# Qwen3-1.7B vocabulary, same chunk_size [2,4,7,10,14,28], same num_delay_frames 3,
# same audio_history_chunks 0, same position scheme, same optimiser, same
# attn_backend, same seed, and NO warm start -- so any delta is the targets.
#
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_tgtfix}"
EXP_NAME="${EXP_NAME:-granary2_script_tgtfix}"
