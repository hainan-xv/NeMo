...
#SBATCH -J nemotron_speechprod_asr:eval-banded-tgtfix
...
# ============================================================================
# The two FIXED-TARGET banded arms, in one job -- 1,024 SentencePiece vs 151,669 Qwen.
#
#   sbatch launch/eval_banded_tgtfix.sh          <- no arguments
#
# Both carry target_construction=partition and delay_word_final_punctuation=true,
# so this is a pure VOCABULARY comparison at a fixed objective. For the
# with-fix/without-fix contrast use launch/eval_banded_vocab_x_tgtfix.sh, which
# runs the same two arms plus their legacy controls.
#
# Each arm is averaged over its top-5 checkpoints by val_wer using NeMo's own
# scripts/checkpoint_averaging/average_model_checkpoints.py, then evaluated on the
# full 74,838-utterance Open-ASR-Leaderboard set through the shared select_shard
# partition, so the numbers are comparable to each other and to the nemotron
# baseline. The averaged .nemo is reused ONLY when it is newer than every
# checkpoint, so an arm that has trained further is re-averaged automatically
# rather than silently evaluated at stale weights.
#
# The arms run SEQUENTIALLY -- each already fans its shard across all 8 GPUs.
# A failure in one does not abandon the other.
#
# EPOCHS ARE NOT MATCHED between the two vocabularies, and both arms are usually
# still training while this runs, so each arm's max epoch and best val_wer are
# printed in its banner. NOTE val_wer here is the ASR collection's VERBATIM
# metric (raw .split() + edit distance, no normalisation) -- it is not comparable
# to speechlm2's val_wer, which Whisper-normalises both sides. Only the
# leaderboard macro below is a cross-family number.
#
# The Qwen arm needs its own config and vocabulary at averaging time: averaging
# constructs the model before loading weights, so a 151,669-piece checkpoint
# built against the 1,024-piece config is a shape error at best.
#
# ENV
