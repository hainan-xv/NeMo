...
# ============================================================================
# SCRIPT multi-lookahead, banded both-side, WITH A 3-FRAME EMISSION DELAY.
#
#   sbatch launch/script_banded1_both_multilookahead_delay3.sh
#
# Delta on script_banded1_both_multilookahead.sh: num_delay_frames 0 -> 3, plus a
# warm start from our own band model instead of the non-banded baseline.
# Everything else -- chunk sizes [2,7,14], band_words 1, band_side both, LR,
# max_duration 20, bucket schedule, 8 nodes -- is held identical, so a delta
# against that arm is the delay alone.
#
# WHY 3. The interleaved SpeechLM baseline now on the official board trains with
# num_delay_frames 3, and at matched lookahead it is level with our arm at
...
# very latency a small chunk exists to save. Read the cs2 row against the
# latency budget, not on WER alone.
# ============================================================================
...
EXP_NAME="${EXP_NAME:-granary2_script_banded1_both_multilookahead_delay3}"
...
# WARM START FROM OUR OWN BAND MODEL, not the non-banded SCRIPT baseline the
# sibling arm uses. That arm had to learn the banded objective from scratch;
# this one starts from weights that already train under it, so the only thing
# left to learn is the delay and the multi-chunk schedule.
#
# HARD-LINKED, NOT REFERENCED IN PLACE. granary2_script_banded1_both is STILL
# TRAINING with save_top_k, so its step=150000 file will eventually be rotated
# away -- and a warm start whose source vanished fails at load, mid-run, hours
# in. The link is the same inode (no extra bytes) and pins the exact weights.
#
# step=150000 (val_wer 0.0962) is that arm's BEST, not its latest: -last was
# 0.0988 at step 152000. For a weights-only init the best checkpoint is the
# right pick; there is no optimizer state to stay consistent with.
INIT_CKPT="${INIT_CKPT:-/lustre/fsw/portfolios/nemotron/users/hainanx/results/SpeechlmOCI/pinned_init/band_both_step150000_valwer0.0962.ckpt}"
INIT_EXP="${INIT_EXP:-}"
INIT_CKPT="${INIT_CKPT:-}"
