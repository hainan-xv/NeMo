...
# ============================================================================
# BASELINE SCRIPT streaming SpeechLM finetune on OCI (draco-oci, IAD).
#
# The clean, vanilla SCRIPT model for the SCRIPT-vs-interleaving comparison:
# NO prompt control, NO self-correction, NO shared/bounded audio. Just the base
# spine+branch objective p(words_k | text_history_<k, audio_k), trained via
# examples/speechlm2/script_train.py (ScriptSTTModel + ScriptSTTDataset) on the
# base recipe streaming_stt_granary2_lora_script.yaml, with three deliberate
# operating-point choices baked in as CLI overrides:
#   * delay        = 3 frames         (data.dataset.num_delay_frames=3; base default)
#   * text format  = caps + punctuation (the Granary v2 en_pnc target text, i.e. the
#                    recipe default -- vary_text_repr stays OFF, so targets keep
#                    normal capitalization and punctuation)
#   * chunk size   = {2, 7, 14, 28}   (model.chunk_size; one drawn per batch)
#
# It also sets a more natural instruction that reads straight into the history:
# the spine is packed as [instruction | history words ...], so ending the prompt
# with "The text history is:" flows directly into the transcript (empty for the
# first chunk).
#
...
EXP_NAME=granary2_script_baseline
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (BASELINE SCRIPT, granary2, no-blank | delay=${DELAY} | caps+punct | chunk-size ${CHUNK_SIZES}) ***" \
...
&& echo "Starting training (running the clean SCRIPT repo at /code, SCRIPT streaming SpeechLM, GRANARY 2.0)" \
