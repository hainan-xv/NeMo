...
#SBATCH -J nemotron_speechprod_asr:dfw-chat-spe8k-bigbatch
#SBATCH -p batch
#SBATCH -N 2
...
# CHAT band-1 both-side, 8k SentencePiece vocabulary, at the x4 batch.
#
# THE BRIDGE ARM. 8k/16k/32k finished at the x2 batch (4.92/4.95/4.95) while
# 1k/2k/4k run at x4, so a 1k-vs-8k difference currently confounds vocabulary
# with batch. This re-runs the best finished vocabulary at the NEW batch, so
# the low end has an anchor measured under identical conditions.
#
...
# NEW EXP NAME, deliberately. dfw_granary2_chat_spe8k_both already holds a
# finished run at the x2 batch (epoch 127, macro-7 4.92); reusing that name
# would RESUME it rather than start this arm.
export EXP_NAME="${EXP_NAME:-dfw_granary2_chat_spe8k_both_bigbatch}"
...
export TOKENIZER_DIR="${DFW}/hainanx/tokenizers/granary2_en_spe/v8192"
