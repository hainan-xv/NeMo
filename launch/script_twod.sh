...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-twod
...
# ============================================================================
# SCRIPT with the 2-D LAYOUT -- a controlled VERIFICATION run.
#
# Identical to launch/script_baseline.sh in every training knob (same chunk
# sizes, same delay, same audio window, same optimizer, same data). The ONLY
# difference is how each training example is arranged:
#
#   baseline: one flat sequence per utterance, with a 4D mask forbidding every
#             cross-branch attention pair -- so those pairs are computed and then
#             thrown away.
#   2-D:      the spine is forwarded once and its K/V shared (broadcast, not
#             copied) by every branch; branches run as a BATCH. The cross-branch
#             pairs are never created.
#
# The layouts are provably equivalent, so THIS RUN SHOULD REPRODUCE THE BASELINE.
# It warm-starts from granary2_script_baseline, so the first logged loss should
# match that run's current loss -- check that before letting it continue. A
# divergence means the layout is wrong, not that the model changed.
#
# Runs the synced repo mounted at /code, NOT the container's bundled NeMo.
#
# Usage (from the repo root on the OCI login node):
#   sbatch launch/script_twod.sh              # seed 42
#   sbatch launch/script_twod.sh 123          # seed 123
#   MICRO_BATCH=16 sbatch launch/script_twod.sh    # smaller groups if memory is tight
#   INIT_CKPT=none sbatch launch/script_twod.sh    # from scratch (compare curves)
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_twod.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_twod.sh
#
# Knobs (env overrides):
...
#   MICRO_BATCH          -- branches per group; activations recomputed in
#                           backward. 0 = all at once. Cannot change the loss,
#                           only memory. (default 32)
...
#
# NOTE: because branches are a batch axis here rather than one long sequence,
# MICRO_BATCH decouples activation memory from utterance length -- something the
# flat layout cannot do. Start at 32; lower it if you hit OOM. It is purely a
# memory/throughput knob and provably does not affect the loss.
# ============================================================================
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_twod}"
EXP_NAME="${EXP_NAME:-granary2_script_twod}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT 2-D layout, granary2, no-blank | delay=${DELAY} | micro_batch=${MICRO_BATCH} | chunk sizes ${CHUNK_SIZES}) ***" \
&& echo "*** OBJECTIVE: p(words_k | text_history_<k, audio_k); 2-D layout -- spine forwarded once, branches batched ***" \
...
&& python /code/examples/speechlm2/script_train.py \
    ...
    model.twod_branch_micro_batch=${MICRO_BATCH} \
