...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-lastlayer-promptctl
...
# ============================================================================
# LAST-LAYER RESTRICTED-HISTORY  x  PROMPT-CONTROLLED SCRIPT streaming SpeechLM
# finetune on OCI (draco-oci, IAD).
#
# Takes the already-trained, NON-prompt-controlled last-layer model
# (granary2_script_lastlayer, trained by launch/script_lastlayer.sh: top
# RESTRICT_LAYERS LLM layer(s) attend only the most recent HISTORY_TOKENS history
# tokens, fixed delay=3, caps+punct) and fine-tunes the PROMPT-CONTROL features in
# on top of it. The restriction knobs are carried over unchanged; the dataset now
# draws + states three knobs per batch (recipe
# streaming_stt_granary2_lora_script_lastlayer_promptctl.yaml, run via
# examples/speechlm2/script_train.py -> ScriptSTTModel + ScriptSTTDataset):
#   * chunk size   -- one of {2, 7, 14, 28} rendered into the prompt
#                     ("Process the audio in chunks of N frames.")
#   * delay        -- exact delay ~ Uniform[0,4] frames, rendered into the prompt
#                     ("... with a fixed delay of N frames."); num_delay_frames=-1
#   * text format  -- one of the 4 (caps on/off) x (punct on/off) combos, stated in
#                     the prompt and applied to the target text (vary_text_repr)
#
# WARM START: weights are AUTO-INITIALIZED from the LATEST checkpoint of INIT_EXP
# (default granary2_script_lastlayer) -- the non-prompt-control last-layer model --
# via ++exp_manager.resume_from_checkpoint. No need to set anything. Override:
...
#
# The last-layer restriction (HISTORY_TOKENS / RESTRICT_LAYERS) MUST match what the
# warm-start checkpoint was trained with, or the loaded weights will not line up
# with the restricted attention path. Defaults (8 / 1) match launch/script_lastlayer.sh.
#
# Self-correction is NOT included here (no <del> token / DAgger / 2-chunk window).
#
...
#   sbatch launch/script_lastlayer_promptctl.sh          # seed 42, init from latest lastlayer ckpt
#   sbatch launch/script_lastlayer_promptctl.sh 123      # seed 123
#   HISTORY_TOKENS=4 sbatch launch/script_lastlayer_promptctl.sh
# ============================================================================
...
# each batch samples an integer delay ~ Uniform[0, exact_max_delay] and renders the
# exact number into the prompt. Set a fixed delay >=0 to disable prompt control.
DELAY=-1
LR=0.0001
WARMUP_STEPS=10000
COMPACT_TEMPLATE=true

# Last-layer restricted-history knobs (see the recipe). MUST match the warm-start
# checkpoint (granary2_script_lastlayer, trained with 8 / 1). Override via env.
...
# Config: OUR SCRIPT last-layer x prompt-control recipe, shipped in the synced repo.
CONFIG_PATH=/code/examples/speechlm2/conf/
CONFIG_NAME=streaming_stt_granary2_lora_script_lastlayer_promptctl

EXP_NAME=granary2_script_lastlayer_promptctl

# Chunk-size set (encoder frames): one is drawn per batch AND rendered into the
# prompt (chunk_size_prompt_template in the recipe). Matches the baseline's set so
# the models cover the same operating points. Delay and cap/punct are also
# prompt-controlled per batch by the recipe (exact_delay, vary_text_repr); this
# launcher does NOT set a custom system_prompt -- the per-batch training prompt is
# generated from the recipe templates, and the recipe's system_prompt is used only
# for validation.
CHUNK_SIZES="[2,7,14,28]"

# Warm-start: init weights from the LATEST checkpoint of INIT_EXP (default: the
# non-prompt-control last-layer model this is built on). If INIT_CKPT is empty it
# is AUTO-RESOLVED below to that exp's most recent checkpoint. Set INIT_CKPT=/path
# to pin one, INIT_EXP=<other_exp> to init from a different run, or INIT_CKPT=none
# to train from the base pretrained LLM+ASR.
INIT_EXP="${INIT_EXP:-granary2_script_lastlayer}"
INIT_CKPT="${INIT_CKPT:-}"
...
# Auto-resolve the init checkpoint from INIT_EXP when not given explicitly. The user
# asked to warm-start from the LATEST last-layer checkpoint, so prefer the rolling
# "-last.ckpt", else the newest "step=..-val_wer=..ckpt" by step, else newest by
# mtime. resume_if_exists=true means this only seeds the FIRST launch; later
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (LAST-LAYER RESTRICTED x PROMPT-CTL SCRIPT, granary2, no-blank | prompt-set delay Uniform[0,4] | prompt-set cap/punct | prompt-set chunk-size ${CHUNK_SIZES}) ***" \
...
&& echo "*** OBJECTIVE: p(words_k | text_history_<k, audio_k); packed spine+branch, single O(L) forward; chunk-size + delay + cap/punct via prompt ***" \
...
&& echo "Starting training (running MY code at /code, SCRIPT last-layer x prompt-control, GRANARY 2.0)" \
