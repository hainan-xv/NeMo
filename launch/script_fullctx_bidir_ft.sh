...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-fullctx-bidir-ft
...
# ============================================================================
# FULL-CONTEXT + BIDIRECTIONAL AUDIO.
#
# The offline model reads its audio causally, so frame t still cannot see past
# its own encoder chunk -- which is why its WER varies with chunk size when it
# ideally should not (6.10 / 5.78 / 5.62 at C=2/7/14, spread 0.48). Unmasking the
# audio block lets a frame attend the whole utterance and pick up the later
# context the encoder denied it.
#
# PREDICTION: the chunk-size spread compresses, most at C=2, C=14 barely moving.
# Evaluate at all three or the result is unreadable:
#     for c in 2 7 14; do
#       ./oci_launch.sh FORCE_AVERAGE=1 MAX_NEW_TOKENS=256 \
#           launch/eval_script.sh granary2_script_fullctx_bidir_ft $c
#     done
# MAX_NEW_TOKENS must be raised (whole transcript, not one chunk) and must come
# AFTER oci_launch.sh -- the env forwarder denylists names matching *TOKEN*.
#
# NOT COMPATIBLE with STATE_MACHINE=1 eval; the model raises rather than decode
# under a rule it was not trained with.
#
...
#   sbatch launch/script_fullctx_bidir_ft.sh         # seed 42
#   sbatch launch/script_fullctx_bidir_ft.sh 123     # seed 123
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_fullctx_bidir_ft.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_fullctx_bidir_ft.sh
#
# Knobs (env overrides):
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_fullctx_bidir}"
EXP_NAME="${EXP_NAME:-granary2_script_fullctx_bidir_ft}"
...
INIT_EXP="${INIT_EXP:-granary2_script_fullctx_ft}"
INIT_CKPT="${INIT_CKPT:-}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT FULL-CONTEXT + BIDIRECTIONAL AUDIO | delay=${DELAY} | attn=${ATTN_BACKEND} ckpt=${ACT_CKPT} | chunk sizes ${CHUNK_SIZES}) ***" \
