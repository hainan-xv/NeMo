...
#SBATCH -J nemotron_speechprod_asr:oci-off-speechlm-cs7
...
#
# CHUNK SIZE 7, from the same checkpoint as the cs14 run.
#
# IN DISTRIBUTION: this model was TRAINED on chunk_size {2,4,7,10,14,28}, so 7 is
# a size it actually saw -- exactly like our SCRIPT multi-lookahead arm, which
# trained on [2,7,14]. That is what makes a per-lookahead comparison between the
# two layouts fair rather than a handicap match.
# ============================================================================
set -uo pipefail
mkdir -p slurm_out
