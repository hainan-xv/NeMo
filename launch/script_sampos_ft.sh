...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-sampos-ft
...
# ============================================================================
# SCRIPT with a SAMPLED POSITION SCHEME (branch vs continuous, per batch).
#
# Same training as launch/script_flex.sh; the one change is how RoPE positions
# are laid out.
#
#   default "branch"     [history] <vs> audio <ve> words   -- the audio sits
#     BETWEEN the history and the words it predicts, so a word is adjacent to
#     words from the same chunk but window+3 away from the previous word if that
#     one came from the previous chunk. The same transcript therefore has
#     different geometry depending on where the chunk boundaries fell.
#
#   here "sampled"       one of the two is drawn PER BATCH, like the chunk size.
#     Caveat: this augments a factor that does not vary at inference, and the two
#     schemes carry DIFFERENT information -- "branch" lets the word-to-word offset
#     reveal a chunk boundary, "continuous" erases it. Measured so far: the two
#     tie, so this is an experiment rather than an expected win.
#
#   for reference "continuous"  the branch is shifted left by window+2, so its predicted
#     words land exactly on their spine positions. Text is one continuous
#     sequence and the chunking is invisible to it; the audio window instead
#     overlaps the tail of the history -- the audio and the words it realises
#     describe the same moment.
#
# Safe because masking no longer depends on RoPE geometry: the mask reads
# `order_ids` (spine index / branch offset), so moving positions cannot change
# who attends to whom, and packed-vs-standalone exactness holds under both
# schemes.
#
...
# training from scratch, so the run only has to learn the gate rather than
# relearn ASR.
#
# This works because <|box_start|> / <|box_end|> are UNUSED SPECIALS ALREADY IN
# QWEN'S VOCABULARY: no embedding resize, so every tensor keeps its shape and a
# plain SCRIPT checkpoint loads cleanly. Adding new tokens (as heh's readwrite
# SpeechLM does) would change the embedding and LM-head shapes and make this
# impossible.
#
# Expect an initial loss spike either way: the gate ids carry base-Qwen
# embeddings that have never meant read/write. Here the gate is branch-only, so only the BRANCH supervision changes: its first
# predicted token becomes <write>/<read> instead of a word or <eot>. The spine
# the model conditions on is byte-identical to what the baseline saw.
#
# INIT_EXP=... picks a different source run; INIT_CKPT=none trains from scratch,
# which reproduces launch/script_flex.sh. Note resume_if_exists=true means INIT_* seeds
# only the FIRST launch -- relaunches resume this run's own checkpoints, so
# changing INIT_EXP later has no effect unless EXP_NAME changes too.
#
# Runs the synced repo mounted at /code, NOT the container's bundled NeMo.
#
# Usage (from the repo root on the OCI login node):
#   sbatch launch/script_sampos_ft.sh         # seed 42
#   sbatch launch/script_sampos_ft.sh 123     # seed 123
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_sampos_ft.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_sampos_ft.sh
#
# Knobs (env overrides):
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_sampos}"
EXP_NAME="${EXP_NAME:-granary2_script_sampos_ft}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT + SAMPLED POSITIONS | delay=${DELAY} | attn=${ATTN_BACKEND} ckpt=${ACT_CKPT} | chunk sizes ${CHUNK_SIZES}) ***" \
