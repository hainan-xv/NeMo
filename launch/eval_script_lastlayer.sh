...
#SBATCH -J nemotron_speechprod_asr:sslm-lb-script-lastlayer
...
# ============================================================================
# Open-ASR-Leaderboard eval for the LAST-LAYER RESTRICTED-HISTORY SCRIPT model
# (granary2_script_lastlayer, trained by launch/script_lastlayer.sh).
#
...
# model's fixed operating point, then execs that backend on the SAME allocation.
#
# The last-layer model has the IDENTICAL inference operating point to the
# baseline SCRIPT model: its delay (3 frames) is baked into the weights (a
# training-time alignment shift, not stated in the prompt), its targets are
# normal caps+punct, and it uses the verbatim baseline training instruction as
# the decode prompt. The only thing that differs is internal: the TOP
# script_last_layer_restrict_num_layers LLM layer(s) attend to just the most
# recent script_last_layer_history_tokens history tokens. Those two knobs are
# persisted in the checkpoint's MODEL config and rebuilt by from_pretrained
# (script_model.py: "Read from the MODEL config so from_pretrained rebuilds it"),
# so there is NOTHING to set here for the restriction -- loading the checkpoint
# reproduces it exactly. As with the baseline, the only operating-point knob at
# inference is the chunk size (must be one the model saw: {2, 7, 14, 28};
# default 14). Sweep it by resubmitting with CHUNK_SIZE=.
#
# Usage (from the NeMo79 repo root on OCI):
#   sbatch launch/eval_script_lastlayer.sh
#   CHUNK_SIZE=7  sbatch launch/eval_script_lastlayer.sh
#   for c in 2 7 14 28; do CHUNK_SIZE=$c sbatch launch/eval_script_lastlayer.sh; done
#   sbatch launch/eval_script_lastlayer.sh --quick_run        # smoke test, 10 utts/ds
#
# Flags:
...
for _arg in "$@"; do
    case "$_arg" in
        ...
        *) echo "WARNING: ignoring unrecognized argument '${_arg}' (lastlayer eval takes no positional args)." >&2 ;;
    esac
done
QUICK_SUFFIX=""
if (( QUICK_RUN )); then
...
# --- Model + run identity ---
EXP_NAME="${EXP_NAME:-granary2_script_lastlayer}"
# The training project (launch/script_lastlayer.sh: PROJECT_NAME=SpeechlmRefactored).
...
# Decode prompt -- MUST be byte-for-byte the training instruction set in
# launch/script_lastlayer.sh (SYSTEM_PROMPT there), which is identical to the
# baseline SCRIPT prompt. The model keys behavior on this exact text; any drift
...
EVAL_TAG="lastlayer${QUICK_SUFFIX}"
# wandb run name encodes the decode config (delay is baked; targets are caps+punct;
# the last-layer restriction is baked into the checkpoint), so the only knob to
# surface is the chunk size. Logged to WANDB_EVAL_PROJECT.
...
echo "==> last-layer restricted SCRIPT leaderboard eval"
echo "    exp_name:      ${EXP_NAME}"
echo "    project:       ${PROJECT}"
echo "    eval_tag:      ${EVAL_TAG}"
echo "    chunk_size:    ${CHUNK_SIZE}"
echo "    restriction:   baked into checkpoint config (rebuilt by from_pretrained)"
