...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-chat-asr-wdelay
...
# ============================================================================
# CHAT, ~1k ASR vocabulary, with a 28-FRAME JOINT WINDOW -- the transducer
# analogue of SCRIPT's win28.
#
#   sbatch launch/chat_train_win28_worddelay.sh        # no arguments, no env vars
#   ./oci_launch.sh launch/chat_train_win28_worddelay.sh
#
...
#
# COMPARE AGAINST: launch/chat_train.sh (identical but
# joint_history_chunks: 0). The recipe body is shared with chat_train.sh, so the
# arms cannot drift apart.
# ============================================================================
set -uo pipefail

export CONFIG_NAME=streaming_stt_granary2_chat_asrvocab_win28_worddelay
...
export EXP_NAME="${EXP_NAME:-granary2_chat_asrvocab_win28_worddelay}"

# The recipe sets num_delay_frames: 0, but chat_train.sh passes
# data.dataset.num_delay_frames=${DELAY} on the command line, and a Hydra CLI
# override beats the config file -- so without this the fixed 3-frame delay
# would silently come back and the run would have BOTH delays.
export DELAY="${DELAY:-0}"
...
echo "==> CHAT, ~1k ASR vocabulary, 28-frame joint window"
