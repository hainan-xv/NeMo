...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-selfcorrect-chunk
...
#
# Same data / infra as launch/script_selfcorrect.sh, but with WHOLE-CHUNK
# self-correction (self_correction_scope=chunk) via a different recipe:
#   entrypoint: examples/speechlm2/script_train.py
#               (ScriptSTTModel + ScriptSTTDataset)
#   recipe:     /code/examples/speechlm2/conf/streaming_stt_granary2_lora_script_selfcorrect_chunk.yaml
#
# SELF-CORRECTION (WHOLE-CHUNK) variant, built on _promptctl and INITIALIZED from
# its checkpoint (set INIT_CKPT below). Same "delete + re-emit" mechanism as
# _selfcorrect, but the correction unit is the ENTIRE previous chunk: each step a
# no-grad teacher-forced pass finds chunks the model mispredicts (any token
# wrong), then the next branch is rebuilt with that wrong chunk as its committed
# history tail and target "<del> w_prev_chunk w_k" (delete the whole last chunk +
# re-emit). Errors are the model's OWN forced-decoding mistakes; stats + a sample
# correction are logged periodically (grep the logs for "[self-correction]"). Also
# keeps the prompt-controlled exact delay (0..4) + text-representation knobs.
#
...
#   sbatch launch/script_selfcorrect_chunk.sh          # seed 42
#   sbatch launch/script_selfcorrect_chunk.sh 123      # seed 123
#
...
# Config: OUR SCRIPT whole-chunk self-correction recipe, shipped in the synced repo at /code.
CONFIG_PATH=/code/examples/speechlm2/conf/
CONFIG_NAME=streaming_stt_granary2_lora_script_selfcorrect_chunk

EXP_NAME=granary2_script_selfcorrect_chunk
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT, SELF-CORRECTION delete-whole-chunk, init=${INIT_CKPT:-none}, granary2, no-blank) ***" \
