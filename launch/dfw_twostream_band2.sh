...
#SBATCH -J nemotron_speechprod_asr:dfw-twostream-band2
...
# ============================================================================
# TWO-STREAM SCRIPT, band_words=2 (+-2 words, both sides), 4 nodes.
#
#   sbatch launch/dfw_twostream_band2.sh
#
# THE POINT. In packed SCRIPT a wider band means more branch segments, so it
# multiplies packed length and the logit tensor with it -- which is why
# chunk_size=2 with band 2 was unrunnable there. Here candidates SHARE per-chunk
# lattice cells, so the band selects a subset rather than adding segments.
#
# Measured locally on the real path (4 chunks, chunk_size 14):
#     band 0  J=1   12 cells   joint len 187
#     band 1  J=3   18 cells   joint len 271
#     band 2  J=5   24 cells   joint len 355
# J grows 5x; cells grow 2x. Packed SCRIPT would grow ~5x, through all 28 layers
# rather than the one layer the joint uses.
#
# SANITY CHECK BUILT IN: widening the band must LOWER the loss at init
# (marginalising over a superset of paths can only raise the likelihood).
# Locally 24.32 -> 22.92 -> 22.25 for band 0/1/2. If this arm starts ABOVE the
# band-0 arm, the extra candidates are not reaching the lattice.
#
# Same chunk_size=14 and same read-out fix as dfw_twostream_band0_v2, so a delta
# against it is the band alone.
# ============================================================================
set -uo pipefail
...
EXP_NAME="${EXP_NAME:-dfw_twostream_band2}"
