...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-contpos-ft
...
# ============================================================================
# SCRIPT with CONTINUOUS TEXT POSITIONS.
#
...
#
#   here "continuous"    the branch is shifted left by window+2, so its predicted
...
#   sbatch launch/script_contpos_ft.sh         # seed 42
#   sbatch launch/script_contpos_ft.sh 123     # seed 123
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_contpos_ft.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_contpos_ft.sh
#
# Knobs (env overrides):
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_contpos}"
EXP_NAME="${EXP_NAME:-granary2_script_contpos_ft}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT + CONTINUOUS POSITIONS | delay=${DELAY} | attn=${ATTN_BACKEND} ckpt=${ACT_CKPT} | chunk sizes ${CHUNK_SIZES}) ***" \
