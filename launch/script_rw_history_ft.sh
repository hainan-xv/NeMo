...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-rw-history-ft
...
#
# Unlike launch/script_rw_history_ft.sh the gate ALSO enters the history, so the
...
#
# WARM START (this is the only difference from launch/script_rw_history.sh).
...
# embeddings that have never meant read/write. Here the gate ALSO enters the history, so the spine itself changes shape -- it now
# grows with the chunk count, not just the word count. That is a bigger
# departure from what the baseline checkpoint was trained on, so expect a
# larger initial loss spike here than in the branch-only variant.
#
# INIT_EXP=... picks a different source run; INIT_CKPT=none trains from scratch,
# which reproduces launch/script_rw_history.sh. Note resume_if_exists=true means INIT_* seeds
...
#   sbatch launch/script_rw_history_ft.sh         # seed 42
#   sbatch launch/script_rw_history_ft.sh 123     # seed 123
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_rw_history_ft.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_rw_history_ft.sh
#
# Knobs (env overrides):
...
EXP_NAME="${EXP_NAME:-granary2_script_rw_history_ft}"
