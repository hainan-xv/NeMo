...
#SBATCH -J nemotron_speechprod_asr:chat-flexdelay-lr4x
...
#
# One learning rate for every parameter, encoder included -- at 4e-3, FOUR TIMES
# the 1e-3 the sibling flexdelay run uses. Deliberately aggressive: the donor
# RNN-T's own schedule peaked at 1.05e-3, so this runs the pretrained encoder
# ~4x hotter than it has ever been trained. Warmup (2500 steps) and the cosine
# schedule are unchanged, so the only difference from
# chat_train_flexdelay.sh is the peak rate.
#
#   sbatch launch/chat_train_flexdelay_lr4x.sh     <- no arguments, no environment to set
#
...
export LR=0.004
export EXP_NAME="${EXP_NAME:-granary2_chat_forced_asrvocab_flexdelay4_lr4e3}"
