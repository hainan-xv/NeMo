...
#SBATCH -J nemotron_speechprod_asr:dfw-chat-b1-both-fullctx-parakeet
#SBATCH -p batch
#SBATCH -N 2
...
# FULL-CONTEXT encoder arm: the same band-1 both-side CHAT objective, but the
# encoder sees the WHOLE utterance instead of a [70,13] streaming window, and is
# initialised from parakeet-tdt-0.6b-v2 rather than the streaming nemotron.
#
# THIS ARM IS NOT STREAMABLE. Full attention plus non-causal downsampling and
# convolution means no output is available until the audio ends. It is a
# reference point -- what the chunked emission objective is worth when the
# representation is unconstrained -- not a deployable configuration.
#
# Held identical to dfw_chat_banded1_both_nodelay_v2 in every other knob (Qwen
# 151k vocab, band_chunks=1, band_side=both, no delay, LR, batch, 2 nodes), so
# the WER delta is attributable to the encoder alone.
#
# chunk_size is set EXPLICITLY in the config: it is normally inferred from the
# encoder's right context, which a full-context encoder does not have.
#
# WATCH MEMORY. The bucket batch sizes were tuned against a [70,13] window; full
# attention is quadratic in utterance length, so the activation cost per sample
# is higher here than anywhere those numbers were measured. If this OOMs, halve
# BUCKET_BATCH_SIZE rather than reasoning about it -- this project has OOM'd four
# times from batch sizes argued rather than measured.
#

# ============================================================================
# CHAT, BIDIRECTIONAL band=1, on the UNMODIFIED alignment, warm-started.
#
#   sbatch launch/dfw_chat_banded1_both_nodelay.sh   <- no arguments
#
#
# BIDIRECTIONAL BAND, on the unmodified alignment. Identical to the one-sided
# dfw_chat_banded1_nodelay.sh in every knob except band_side=both, so a delta
...
#
# On CHAT the second side is also CHEAP: measured 0.270 s/step two-sided vs
# 0.290 one-sided, i.e. free within noise, because widening the band inflates
# only the loss's node set and not the sequence the model sees. SCRIPT pays
# properly for it (0.212 -> 0.337), so this arm is the cleaner of the two tests.
#
...
export CONFIG_NAME=nemotron_chat_transducer_granary2_qwen_fullctx
export EXP_NAME="${EXP_NAME:-dfw_granary2_chat_banded1_both_fullctx_parakeet}"
...
# starting FOR.
# ENCODER ONLY. parakeet-tdt-0.6b-v2 is a TDT model over a 1024-piece
# SentencePiece vocabulary; its prediction network and joint are shaped for
# that vocab and for duration outputs this model does not have, so they cannot
# transfer. Widening this list would not warm-start more, it would fail to
# match and leave the tensors at init while reporting success.
export INIT_INCLUDE='["encoder."]'
export INIT_EXCLUDE='[]'

# parakeet-tdt-0.6b-v2, staged from HuggingFace. A .nemo because
# init_from_nemo_model is what the CHAT trainer exposes. Under hainanx/, not
# users/heh/, because that tree is not ours to write to.
