...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-fullctx-ft
...
# ============================================================================
# FULL-CONTEXT (OFFLINE) SCRIPT -- the upper bound for the streaming models.
#
# The LLM sees the WHOLE encoded utterance at once and emits the entire
# transcript in one turn; layout-wise, SCRIPT with a single chunk.
#
# The ENCODER is unchanged: att_context_size still follows the sampled chunk size,
# so frames stay chunk-limited and streaming-equivalent. The gap to streaming
# SCRIPT is therefore the cost of EMITTING INCREMENTALLY, not of restricted
...
#   sbatch launch/script_fullctx_ft.sh         # seed 42
#   sbatch launch/script_fullctx_ft.sh 123     # seed 123
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_fullctx_ft.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_fullctx_ft.sh
#
# Knobs (env overrides):
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_fullctx}"
EXP_NAME="${EXP_NAME:-granary2_script_fullctx_ft}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT FULL-CONTEXT / OFFLINE | delay=${DELAY} | attn=${ATTN_BACKEND} ckpt=${ACT_CKPT} | chunk sizes ${CHUNK_SIZES}) ***" \
