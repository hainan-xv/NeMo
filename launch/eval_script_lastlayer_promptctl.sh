...
#SBATCH -J nemotron_speechprod_asr:sslm-lb-script-lastlayer-promptctl
...
# ============================================================================
# Open-ASR-Leaderboard eval for the LAST-LAYER x PROMPT-CONTROLLED SCRIPT model
# (granary2_script_lastlayer_promptctl, trained by launch/script_lastlayer_promptctl.sh).
#
...
# This matches the recipe's validation system_prompt for (delay=3, cap+punct,
# chunk=14) in streaming_stt_granary2_lora_script_lastlayer_promptctl.yaml.
#
# The last-layer restriction (script_last_layer_history_tokens /
# script_last_layer_restrict_num_layers) is a MODEL-side knob persisted in the
# checkpoint config and rebuilt by from_pretrained -- there is NOTHING to set
# here for it. Loading the checkpoint reproduces it exactly.
#
# Usage (from the NeMo79 repo root on OCI):
#   sbatch launch/eval_script_lastlayer_promptctl.sh [cap] [punct] [delay] [chunk]
...
#
# Examples:
#   sbatch launch/eval_script_lastlayer_promptctl.sh                      # cap punct, delay 3, chunk 14
#   sbatch launch/eval_script_lastlayer_promptctl.sh cap   punct   0  7   # low-latency, chunk 7
#   sbatch launch/eval_script_lastlayer_promptctl.sh nocap nopunct 4  28  # high-accuracy, chunk 28
#   for d in 0 2 3 4; do sbatch launch/eval_script_lastlayer_promptctl.sh cap punct $d 14; done
#   CHUNK_SIZE=7 sbatch launch/eval_script_lastlayer_promptctl.sh         # env form of the same knob
#   sbatch launch/eval_script_lastlayer_promptctl.sh --quick_run cap punct 3 14   # smoke test
#
...
if (( DELAY < 0 || DELAY > MAX_DELAY )); then
    echo "ERROR: delay=$DELAY out of range for granary2_script_lastlayer_promptctl (allowed 0..${MAX_DELAY})" >&2
    exit 1
fi
...
# (chunk_size_prompt_template in streaming_stt_granary2_lora_script_lastlayer_promptctl.yaml).
# Must match the recipe validation system_prompt for (delay=3, cap+punct, chunk=14).
...
EXP_NAME="${EXP_NAME:-granary2_script_lastlayer_promptctl}"
# The training project (launch/script_lastlayer_promptctl.sh: PROJECT_NAME=SpeechlmRefactored).
PROJECT="${PROJECT:-SpeechlmRefactored}"
MODEL_CLASS="${MODEL_CLASS:-nemo.collections.speechlm2.models.script_model.ScriptSTTModel}"
# Distinguishes concurrent lastlayer-promptctl evals in RESULTS_DIR (backend adds _chunk<n>).
...
echo "==> last-layer x prompt-controlled SCRIPT leaderboard eval"
