...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-bidiraudio-ft
...
# ============================================================================
# SCRIPT with BIDIRECTIONAL AUDIO inside each branch.
#
# A branch's audio block attends itself both ways, so an early frame in a chunk
# sees a later one; text after the audio stays causal. Streaming-legal (the whole
# block has arrived when the branch starts) and free in FLOPs (the block was
# already computed and then masked). The encoder is untouched.
#
# See the recipe header for what this does and does not buy -- in short, the
# encoder is already bidirectional within a chunk, so this refines the audio
# keys/values the text reads rather than adding acoustic information.
#
# EVALUATE where the audio block is large but the models still separate: a big
# audio window at a small chunk. With this recipe (no window) the honest first
# check is chunk 14 vs the baseline at chunk 14.
#
...
#   sbatch launch/script_bidiraudio_ft.sh         # seed 42
#   sbatch launch/script_bidiraudio_ft.sh 123     # seed 123
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_bidiraudio_ft.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_bidiraudio_ft.sh
#
# Knobs (env overrides):
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_bidiraudio}"
EXP_NAME="${EXP_NAME:-granary2_script_bidiraudio_ft}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT + BIDIRECTIONAL AUDIO | delay=${DELAY} | attn=${ATTN_BACKEND} ckpt=${ACT_CKPT} | chunk sizes ${CHUNK_SIZES}) ***" \
