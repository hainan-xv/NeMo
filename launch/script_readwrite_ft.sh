...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-readwrite-ft
...
# ============================================================================
# SCRIPT with an EXPLICIT READ/WRITE GATE.
#
...
#
# The gate is BRANCH-ONLY: the spine remains the instruction plus the words, so
# unlike an interleaved model this one does not condition on its own past gate
# decisions. That is deliberate -- a gate in the spine would break the exactness
# property the parity tests check.
#
...
#
# WARM START (this is the only difference from launch/script_readwrite.sh).
...
# which reproduces launch/script_readwrite.sh. Note resume_if_exists=true means INIT_* seeds
...
#   sbatch launch/script_readwrite_ft.sh         # seed 42
#   sbatch launch/script_readwrite_ft.sh 123     # seed 123
#
# Or from your laptop:
#   ./oci_launch.sh launch/script_readwrite_ft.sh
#   ./oci_launch_interactive.sh MAX_STEPS=50 VAL_CHECK_INTERVAL=25 launch/script_readwrite_ft.sh
#
# Knobs (env overrides):
...
CONFIG_NAME="${CONFIG_NAME:-streaming_stt_granary2_lora_script_readwrite}"
EXP_NAME="${EXP_NAME:-granary2_script_readwrite_ft}"
...
read -r -d '' cmd <<EOF
echo "*******STARTING********" \
&& echo "*** RECIPE: ${CONFIG_NAME} (SCRIPT + READ/WRITE GATE | delay=${DELAY} | attn=${ATTN_BACKEND} ckpt=${ACT_CKPT} | chunk sizes ${CHUNK_SIZES}) ***" \
