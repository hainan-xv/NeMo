...
# ============================================================================
# CACHE-AWARE STREAMING leaderboard eval for the NEMOTRON 3.5 streaming RNNT model
...
#
# This is a thin wrapper around eval_nemotron_streaming.sh (which decodes with
# NeMo's cache-aware streaming entrypoint -- the CORRECT path for this streaming
# model; the offline examples/asr/speech_to_text_eval.py path SIGFPEs on it -- and
# re-scores with scripts/rescore_nemo_eval.py for a leaderboard-faithful WER). It
# ONLY pins two things -- the .nemo model and the DATASETS suite -- so you get the
# streaming RNNT number on the SAME datasets the script/flush oldsuite runs are
# scored on (see eval_flush_model_oldsuite.sh's aggregate.log), for an
...
#
# All other knobs (NEMO_MODEL, CACHE_DIR, BATCH_SIZE, MAX_SAMPLES, ATT_CONTEXT_SIZE,
# CHUNK_SIZE, CONTAINER, CODE_DIR, OUTPUT_PREFIX, ...) are env vars handled by the
# backend -- see eval_nemotron_streaming.sh's header.
# ============================================================================
set -euo pipefail
...
echo "==> Nemotron 3.5 RNNT cache-aware streaming eval on OLD 8-set suite (delegating to eval_nemotron_streaming.sh)"
echo "    nemo_model: ${NEMO_MODEL}"
echo "    datasets:   ${DATASETS}"
echo "    exp_name:   ${EXP_NAME}"

# Locate eval_nemotron_streaming.sh. Under sbatch, Slurm COPIES this script into a
# spool dir, so BASH_SOURCE is unreliable -- prefer SLURM_SUBMIT_DIR (cwd at submit).
resolve_launch_dir() {
    if [[ -n "${SLURM_SUBMIT_DIR:-}" ]]; then
        [[ -f "${SLURM_SUBMIT_DIR}/eval_nemotron_streaming.sh" ]] && { echo "${SLURM_SUBMIT_DIR}"; return; }
        [[ -f "${SLURM_SUBMIT_DIR}/launch/eval_nemotron_streaming.sh" ]] && { echo "${SLURM_SUBMIT_DIR}/launch"; return; }
    fi
    local here; here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    [[ -f "${here}/eval_nemotron_streaming.sh" ]] && { echo "${here}"; return; }
    echo "ERROR: cannot locate eval_nemotron_streaming.sh (SLURM_SUBMIT_DIR=${SLURM_SUBMIT_DIR:-<unset>})" >&2
    exit 1
}
LAUNCH_DIR="$(resolve_launch_dir)"
cd "${SLURM_SUBMIT_DIR:-$LAUNCH_DIR}"

# eval_nemotron_streaming.sh takes the .nemo path as $1; DATASETS/EXP_NAME flow as env.
exec bash "${LAUNCH_DIR}/eval_nemotron_streaming.sh" "${NEMO_MODEL}"
