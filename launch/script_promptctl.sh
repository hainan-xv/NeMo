...
#
# The flexible counterpart to script_baseline.sh for the SCRIPT-vs-interleaving
# pitch: ONE model whose streaming operating point is dialed entirely through the
# prompt. Same spine+branch objective p(words_k | text_history_<k, audio_k),
# trained via examples/speechlm2/script_train.py (ScriptSTTModel + ScriptSTTDataset)
# on recipe streaming_stt_granary2_lora_script_promptctl.yaml, which surfaces
# three knobs in the prompt (each drawn per batch by ScriptSTTDataset):
...
# a later variant adds a self-correction suffix on top of this prompt-control base.
# Because the prompt is generated per batch from the recipe's templates, this
# launcher does NOT override data.dataset.system_prompt (that value is only the
...
CONFIG_NAME=streaming_stt_granary2_lora_script_promptctl
...
# the two models cover the same operating points. Delay and cap/punct are also
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (PROMPT-CTL SCRIPT, granary2, no-blank | prompt-set delay Uniform[0,4] | prompt-set cap/punct | prompt-set chunk-size ${CHUNK_SIZES} | no self-correction) ***" \
