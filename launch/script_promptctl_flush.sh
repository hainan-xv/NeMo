...
#SBATCH -J nemotron_speechprod_asr:streaming-stt-script-promptctl-flush
...
# ============================================================================
# PROMPT-CONTROLLED SCRIPT streaming SpeechLM finetune, WITH <flush> (draco-oci).
#
# Same recipe/knobs as launch/script_promptctl.sh (chunk-size / emission-delay /
# cap+punct all dialed through the prompt), plus a dedicated <flush> control token
# that fixes the inference "tail-drop" bug: with a positive emission delay, words
# whose audio ends near the clip boundary are held past the end of the audio and
# never emitted, so they show up as DELETIONS at the end of each utterance (WER
# gets worse as the requested delay grows).
#
# The <flush> token tells the model to emit EVERY not-yet-emitted word whose audio
# has ended, DISREGARDING the delay. It is placed right after <ve> in a chunk's
# branch ([<vs> A_k .. <ve> <flush> w_k <eot>]) and is an INPUT signal only (the
# model is never trained to emit it). Training supervision (ScriptSTTDataset):
#   * the FINAL chunk of every utterance ALWAYS flushes (the end-of-audio drain),
#   * each NON-final chunk flushes independently with prob FLUSH_PROB (so the model
#     also learns on-demand mid-utterance draining), and empty flushes are allowed.
# At inference, generate() appends <flush> on each stream's final chunk (baked into
# the checkpoint cfg via ++model.flush, so eval auto-enables it).
#
# <flush> reuses an unused Qwen special token (FLUSH_TOKEN) -- NO embedding resize
# or checkpoint surgery -- so this run WARM-STARTS from the already-trained prompt
# control model (granary2_script_promptctl) and only has to learn the flush
# behavior on top of the delay conditioning it already knows.
#
...
#   sbatch launch/script_promptctl_flush.sh          # seed 42
#   sbatch launch/script_promptctl_flush.sh 123       # seed 123
#
# Knobs (env overrides):
...
EXP_NAME=granary2_script_promptctl_flush
...
# NOTE: the prompt wording is UNCHANGED from the non-flush run -- <flush> is a
# control token injected in the branch layout, not a prompt clause.
...
# <flush> is a small add-on to the prompt-control model (which already conditions on
# the delay clause) and reuses an unused special token, so warm-start from
# granary2_script_promptctl by default. If INIT_CKPT is empty it is AUTO-RESOLVED
# below to that exp's most recent checkpoint. Override: INIT_CKPT=/path/to.ckpt to
# pin one, INIT_EXP=<other_exp> to init from a different run (e.g.
# granary2_script_baseline), or INIT_CKPT=none to train from the base pretrained
# LLM+ASR. The init run MUST share this model's architecture (same base recipe /
...
echo "*******STARTING********" \
&& echo "---------------" \
&& echo "*** RECIPE: ${CONFIG_NAME} (PROMPT-CONTROLLED SCRIPT + <flush>, granary2, no-blank | chunk-size ${CHUNK_SIZES} via prompt | delay ~ U[0,${EXACT_MAX_DELAY}] via prompt | cap/punct on/off via prompt | flush_token=${FLUSH_TOKEN} flush_prob=${FLUSH_PROB}) ***" \
&& echo "*** WARM START: init=${INIT_CKPT:-none} (from ${INIT_EXP}) ***" \
&& echo "*** OBJECTIVE: p(words_k | text_history_<k, audio_k) with end-of-audio + on-demand <flush> drain; packed spine+branch, single O(L) forward ***" \
...
&& echo "Starting training (running the clean SCRIPT repo at /code, PROMPT-CONTROLLED SCRIPT + <flush> streaming SpeechLM, GRANARY 2.0)" \
